...
export async function getCompany_GoalSummary(req: Request): Promise<IResponse> {
    return withDB(async () => {
        const now = new Date();
        const twentyFourHoursAgo = new Date(now.getTime() - 24 * 60 * 60 * 1000);

        const [totalDocs, last24HoursDocs] = await Promise.all([
            Company_Goal.countDocuments({}),
            Company_Goal.countDocuments({ createdAt: { $gte: twentyFourHoursAgo } }),
        ]);
