...
export async function getFollow_UpSummary(req: Request): Promise<IResponse> {
    return withDB(async () => {
        const now = new Date();
        const twentyFourHoursAgo = new Date(now.getTime() - 24 * 60 * 60 * 1000);

        const [totalDocs, last24HoursDocs] = await Promise.all([
            Follow_Up.countDocuments({}),
            Follow_Up.countDocuments({ createdAt: { $gte: twentyFourHoursAgo } }),
        ]);
