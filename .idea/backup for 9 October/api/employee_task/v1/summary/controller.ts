...
export async function getEmployee_TaskSummary(req: Request): Promise<IResponse> {
    return withDB(async () => {
        const now = new Date();
        const twentyFourHoursAgo = new Date(now.getTime() - 24 * 60 * 60 * 1000);

        const [totalDocs, last24HoursDocs] = await Promise.all([
            Employee_Task.countDocuments({}),
            Employee_Task.countDocuments({ createdAt: { $gte: twentyFourHoursAgo } }),
        ]);
