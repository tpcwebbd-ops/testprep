...
export async function getContent_ResourcSummary(req: Request): Promise<IResponse> {
    return withDB(async () => {
        const now = new Date();
        const twentyFourHoursAgo = new Date(now.getTime() - 24 * 60 * 60 * 1000);

        const [totalDocs, last24HoursDocs] = await Promise.all([
            Content_Resourc.countDocuments({}),
            Content_Resourc.countDocuments({ createdAt: { $gte: twentyFourHoursAgo } }),
        ]);
