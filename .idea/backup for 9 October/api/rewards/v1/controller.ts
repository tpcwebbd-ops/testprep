...
// CREATE Reward
export async function createReward(req: Request): Promise<IResponse> {
    return withDB(async () => {
        try {
...
export async function getRewardById(req: Request): Promise<IResponse> {
    return withDB(async () => {
        const id = new URL(req.url).searchParams.get('id')
        if (!id)
            return formatResponse(null, 'Reward ID is required', 400)

        const reward = await Reward.findById(id)
        if (!reward)
            return formatResponse(null, 'Reward not found', 404)
...
// GET all Rewards with pagination and intelligent search
export async function getRewards(req: Request): Promise<IResponse> {
    return withDB(async () => {
        ...
        if (searchQuery) {
            ...
            if (searchQuery.startsWith('createdAt:range:')) {
                ...
                const stringFields = ["user_id","user_Name","given_by_id","given_by_Name","reason"];
        ...
        const rewards = await Reward.find(searchFilter)
            .sort({ updatedAt: -1, createdAt: -1 })
            .skip(skip)
            .limit(limit)

        const totalRewards =
            await Reward.countDocuments(searchFilter)
...
// UPDATE single Reward by ID
export async function updateReward(req: Request): Promise<IResponse> {
    return withDB(async () => {
        try {
...
// BULK UPDATE Rewards
export async function bulkUpdateRewards(req: Request): Promise<IResponse> {
    return withDB(async () => {
        const updates: { id: string; updateData: Record<string, unknown> }[] = await req.json()
        const results = await Promise.allSettled(
            updates.map(({ id, updateData }) =>
                Reward.findByIdAndUpdate(id, updateData, {
                    new: true,
                    runValidators: true,
                })
            )
...
// DELETE single Reward by ID
export async function deleteReward(req: Request): Promise<IResponse> {
    return withDB(async () => {
...
// BULK DELETE Rewards
export async function bulkDeleteRewards(req: Request): Promise<IResponse> {
    return withDB(async () => {
        ...
        for (const id of ids) {
            try {
                ...
                if (doc) {
                    const deletedDoc = await Reward.findByIdAndDelete(id)
                    if (deletedDoc) {
                        deletedIds.push(id)
                    }
                } else {
