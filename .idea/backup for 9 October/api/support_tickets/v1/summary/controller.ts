...
export async function getSupport_TicketSummary(req: Request): Promise<IResponse> {
    return withDB(async () => {
        const now = new Date();
        const twentyFourHoursAgo = new Date(now.getTime() - 24 * 60 * 60 * 1000);

        const [totalDocs, last24HoursDocs] = await Promise.all([
            Support_Ticket.countDocuments({}),
            Support_Ticket.countDocuments({ createdAt: { $gte: twentyFourHoursAgo } }),
        ]);
