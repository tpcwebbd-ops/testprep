...
type DisplayableRewardsKeys = 
    ...
    | 'reward_time'
    | 'createdAt'
type ColumnVisibilityState = Record<DisplayableRewardsKeys, boolean>


const ViewTableNextComponents: React.FC = () => {
    const [sortConfig, setSortConfig] = useState<{
        key: DisplayableRewardsKeys
        direction: 'asc' | 'desc'
    } | null>(null)
    ...
    const allData = useMemo(
        () => getResponseData?.data?.rewards || [],
        [getResponseData]
    )

    const tableHeaders: { key: DisplayableRewardsKeys; label: string }[] = [
        { key: 'user_id', label: 'User_id' },
        { key: 'user_Name', label: 'User_ Name' },
        { key: 'given_by_id', label: 'Given_by_id' },
        { key: 'given_by_Name', label: 'Given_by_ Name' },
        { key: 'reward_date', label: 'Reward_date' },
        { key: 'reward_time', label: 'Reward_time' },
    ...
    const handleSort = (key: DisplayableRewardsKeys) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: IRewards) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const renderActions = (item: IRewards) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedRewards(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedRewards(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedRewards(item); toggleDeleteModal(true); }}>
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <ExportDialog
                ...
                data={bulkData}
                fileName={`Exported_Rewards_${new Date().toISOString()}.xlsx`}
            />
        </div>
    )
}
