...
import { useBulkDeleteRewardsMutation } from '../redux/rtk-api'
...
const BulkDeleteNextComponents: React.FC = () => {
    ...
    const [bulkDeleteRewards, { isLoading }] = useBulkDeleteRewardsMutation()

    const handleBulkDelete = async () => {
        if (!bulkData?.length) return
        try {
            const ids = bulkData.map((rewards) => rewards._id)
            await bulkDeleteRewards({ ids }).unwrap()
            ...
            <DialogContent>
                ...
                <ScrollArea className="h-[400px] w-full rounded-md border p-4">
                    <div className="flex flex-col">
                        {bulkData.map((rewards, idx) => (
                            <span
                                key={(rewards._id as string) + idx}
                                className="text-xs"
                            >
                                {idx + 1}. {(rewards)['user_id'] as string || ''}
                            </span>
                        ))}
                    </div>
                </ScrollArea>
