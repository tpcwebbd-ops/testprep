...
import { useBulkDeleteLecturesMutation } from '../redux/rtk-api'
...
const BulkDeleteNextComponents: React.FC = () => {
    ...
    const [bulkDeleteLectures, { isLoading }] = useBulkDeleteLecturesMutation()

    const handleBulkDelete = async () => {
        if (!bulkData?.length) return
        try {
            const ids = bulkData.map((lectures) => lectures._id)
            await bulkDeleteLectures({ ids }).unwrap()
            ...
            <DialogContent>
                ...
                <ScrollArea className="h-[400px] w-full rounded-md border p-4">
                    <div className="flex flex-col">
                        {bulkData.map((lectures, idx) => (
                            <span
                                key={(lectures._id as string) + idx}
                                className="text-xs"
                            >
                                {idx + 1}. {(lectures)['title'] as string || ''}
                            </span>
                        ))}
                    </div>
                </ScrollArea>
