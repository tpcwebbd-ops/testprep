...
const ViewTableNextComponents: React.FC = () => {
    ...
    const allData = useMemo(
        () => getResponseData?.data?.lectures || [],
        [getResponseData]
    )

    const tableHeaders: { key: DisplayableLecturesKeys; label: string }[] = [
        { key: 'module_id', label: 'Module_id' },
        { key: 'title', label: 'Title' },
        { key: 'lecture_date', label: 'Lecture_date' },
        { key: 'order_index', label: 'Order_index' },
    ...
    const handleSort = (key: DisplayableLecturesKeys) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: ILectures) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const renderActions = (item: ILectures) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedLectures(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedLectures(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedLectures(item); toggleDeleteModal(true); }}>
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <ExportDialog
                ...
                data={bulkData}
                fileName={`Exported_Lectures_${new Date().toISOString()}.xlsx`}
            />
        </div>
    )
}
