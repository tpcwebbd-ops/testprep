...
const Page = async () => {
    const fetchData = async () => {
        ...
        const url =
            'http://localhost:3000/generate/attendance_s/all/api/v1?page=1&limit=4'
        ...
        try {
            ...
            return responseData.data?.attendance_s
        } catch (error) {
            ...
            {data &&
                ...
                data.map((i: { user_id: string; _id: string }, idx: number) => (
                    <div key={idx + i?.user_id}>
                        <CustomLInk
                            name={i.user_id}
                            url={`/generate/attendance_s/ssr-view/details/${i._id}`}
                        />
                    </div>
                ))}
        </main>
