...
import { useGetAttendance_sQuery } from '../redux/rtk-api'
import Pagination from './Pagination'
import ExportDialog from './ExportDialog' // Import the new dialog component

// Dynamically generated types for type safety
type DisplayableAttendance_sKeys = 
    | 'user_id'
    | 'date'
    | 'time'
    | 'createdAt'
type ColumnVisibilityState = Record<DisplayableAttendance_sKeys, boolean>


const ViewTableNextComponents: React.FC = () => {
    const [sortConfig, setSortConfig] = useState<{
        key: DisplayableAttendance_sKeys
        direction: 'asc' | 'desc'
    } | null>(null)
    ...
    const allData = useMemo(
        () => getResponseData?.data?.attendance_s || [],
        [getResponseData]
    )

    const tableHeaders: { key: DisplayableAttendance_sKeys; label: string }[] = [
    ...
    const handleSort = (key: DisplayableAttendance_sKeys) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: IAttendance_s) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const renderActions = (item: IAttendance_s) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedAttendance_s(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedAttendance_s(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedAttendance_s(item); toggleDeleteModal(true); }}>
    ...
    const renderTableRows = () =>
        sortedData.map((item: IAttendance_s) => (
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <ExportDialog
                ...
                data={bulkData}
                fileName={`Exported_Attendance_s_${new Date().toISOString()}.xlsx`}
            />
        </div>
    )
}
