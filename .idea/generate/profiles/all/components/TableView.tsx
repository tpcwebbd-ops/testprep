...
import { useGetProfilesQuery } from '../redux/rtk-api'
import Pagination from './Pagination'
import ExportDialog from './ExportDialog' // Import the new dialog component

// Dynamically generated types for type safety
type DisplayableProfilesKeys = 
    | 'user_id'
    | 'address'
    | 'social_links'
    | 'createdAt'
type ColumnVisibilityState = Record<DisplayableProfilesKeys, boolean>


const ViewTableNextComponents: React.FC = () => {
    const [sortConfig, setSortConfig] = useState<{
        key: DisplayableProfilesKeys
        direction: 'asc' | 'desc'
    } | null>(null)
    ...
    const allData = useMemo(
        () => getResponseData?.data?.profiles || [],
        [getResponseData]
    )

    const tableHeaders: { key: DisplayableProfilesKeys; label: string }[] = [
        { key: 'user_id', label: 'User_id' },
        { key: 'address', label: 'Address' },
        { key: 'social_links', label: 'Social_links' },
    ...
    const handleSort = (key: DisplayableProfilesKeys) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: IProfiles) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const renderActions = (item: IProfiles) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedProfiles(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedProfiles(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedProfiles(item); toggleDeleteModal(true); }}>
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <ExportDialog
                ...
                data={bulkData}
                fileName={`Exported_Profiles_${new Date().toISOString()}.xlsx`}
            />
        </div>
    )
}
