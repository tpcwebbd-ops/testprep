...
type DisplayablePosts_nKeys = 
    ...
    | 'checked_by_user_id'
    | 'createdAt'
type ColumnVisibilityState = Record<DisplayablePosts_nKeys, boolean>


const ViewTableNextComponents: React.FC = () => {
    const [sortConfig, setSortConfig] = useState<{
        key: DisplayablePosts_nKeys
        direction: 'asc' | 'desc'
    } | null>(null)
    ...
    const allData = useMemo(
        () => getResponseData?.data?.posts_n || [],
        [getResponseData]
    )

    const tableHeaders: { key: DisplayablePosts_nKeys; label: string }[] = [
    ...
    const handleSort = (key: DisplayablePosts_nKeys) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: IPosts_n) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const renderActions = (item: IPosts_n) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedPosts_n(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedPosts_n(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedPosts_n(item); toggleDeleteModal(true); }}>
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <ExportDialog
                ...
                data={bulkData}
                fileName={`Exported_Posts_n_${new Date().toISOString()}.xlsx`}
            />
        </div>
    )
}
