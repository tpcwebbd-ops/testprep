...
import { marketing_LeadsSelectorArr } from '../store/store-constant'
import { useBulkUpdateMarketing_LeadsMutation } from '../redux/rtk-api'
...
const BulkUpdateNextComponents: React.FC = () => {
    ...
    const [bulkUpdateMarketing_Leads, { isLoading }] = useBulkUpdateMarketing_LeadsMutation()
    ...
    const handleBulkUpdate = async () => {
        ...
        try {
            ...
            await bulkUpdateMarketing_Leads(newBulkData).unwrap()
            toggleBulkUpdateModal(false)
            setBulkData([])
            handleSuccess('Update Successful')
        } catch (error) {
            console.error('Failed to edit marketing_Leads:', error)
            ...
            <DialogContent>
                    ...
                    <div>
                        ...
                        <div className="w-full flex items-center justify-between pt-2">
                            ...
                            <Select
                                ...
                                defaultValue={
                                    (marketing_LeadsSelectorArr[0] as string) || ''
                                }
                            >
                                ...
                                <SelectContent className="bg-slate-50">
                                    {marketing_LeadsSelectorArr?.map((option, index) => (
                                        <SelectItem
                                            key={option + index}
                                            value={option}
                ...
                <ScrollArea className="h-[400px] w-full rounded-md border p-4">
                    <div className="flex flex-col gap-2">
                        {bulkData.map((marketing_lead, idx) => (
                            <div
                                key={(marketing_lead._id as string) || idx}
                                className="flex items-center justify-between"
                            >
                                <span>
                                    {idx + 1}. {(marketing_lead)['full_name'] as string || ''}
                                </span>
                                <span className="text-blue-500">{marketing_lead['full_name'] as string}</span>
                            </div>
                        ))}
                    </div>
                </ScrollArea>
