...
const Page = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const url =
                'http://localhost:3000/dashboard/support_Ticket/all/api/v1?page=1&limit=4'
            ...
            try {
                ...
                setData(responseData?.data?.support_Tickets || [])
            } catch (error) {
        ...
        <main className="w-full flex flex-col gap-2 p-1 md:p-4">
            {data?.map((item: DataItem, idx: number) => (
                <div key={idx + item?._id}>
                    <CustomLInk
                        name={item.title}
                        url={`/dashboard/support_Ticket/client-view/details/${item._id}`}
                    />
                </div>
            ))}
        </main>
