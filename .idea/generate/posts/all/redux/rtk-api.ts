...
export const postsApi = apiSlice.injectEndpoints({
    endpoints: (builder) => ({
        getPosts: builder.query({
            query: ({ page, limit, q }) => {
                let url = `/generate/posts/all/api/v1?page=${page || 1}&limit=${limit || 10}`
                if (q) {
        ...
        getPostsSummary: builder.query({
            query: ({ page, limit }) => {
                return `/generate/posts/all/api/v1/summary?page=${page || 1}&limit=${limit || 10}`
            },
            providesTags: [{ type: 'tagTypePosts', id: 'LIST' }],
        }),
        getPostsById: builder.query({
            query: (id) => `/generate/posts/all/api/v1?id=${id}`,
        }),
        addPosts: builder.mutation({
            query: (newPost) => ({
                url: '/generate/posts/all/api/v1',
                method: 'POST',
                body: newPost,
            }),
        ...
        bulkUpdatePosts: builder.mutation({
            query: (bulkData) => ({
                url: `/generate/posts/all/api/v1?bulk=true`,
                method: 'PUT',
                body: bulkData,
            }),
            invalidatesTags: [{ type: 'tagTypePosts' }],
