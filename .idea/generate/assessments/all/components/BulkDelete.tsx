...
import { useBulkDeleteAssessmentsMutation } from '../redux/rtk-api'
...
const BulkDeleteNextComponents: React.FC = () => {
    ...
    const [bulkDeleteAssessments, { isLoading }] = useBulkDeleteAssessmentsMutation()

    const handleBulkDelete = async () => {
        if (!bulkData?.length) return
        try {
            const ids = bulkData.map((assessments) => assessments._id)
            await bulkDeleteAssessments({ ids }).unwrap()
            ...
            <DialogContent>
                ...
                <ScrollArea className="h-[400px] w-full rounded-md border p-4">
                    <div className="flex flex-col">
                        {bulkData.map((assessments, idx) => (
                            <span
                                key={(assessments._id as string) + idx}
                                className="text-xs"
                            >
                                {idx + 1}. {(assessments)['title'] as string || ''}
                            </span>
                        ))}
                    </div>
                </ScrollArea>
