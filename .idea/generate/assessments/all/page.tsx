...
import ViewAssessmentsTable from './components/TableView';
import BulkUpdateAssessments from './components/BulkUpdate';
import BulkDynamicUpdateAssessments from './components/BulkDynamicUpdate';
...
const MainNextPage: React.FC = () => {
  ...
  const { toggleAddModal, queryPramsLimit, queryPramsPage, queryPramsQ, setQueryPramsPage, setQueryPramsQ } = useAssessmentsStore();
  ...
  const handleApplyFilter = (filter: FilterPayload) => {
    ...
    logger.info('Applying filter:', filterQuery);
  ...
  const modals = [AddFile, ViewFile, BulkDeleteFile, BulkEditFile, EditFile, DeleteFile, BulkUpdateAssessments, BulkDynamicUpdateAssessments];

  let renderUI = (
    <div className="container mx-auto p-4">
      <div className="flex flex-col md:flex-row gap-2 justify-between items-center mb-6">
        <h1 className="h2 w-full">Assessment Management {isSuccess && <sup className="text-xs">(total:{getResponseData?.data?.total || '00'})</sup>}</h1>
        <div className="w-full flex flex-col md:flex-row gap-2 item-center justify-end">
          <Summary />
          <Button size="sm" variant="outline" onClick={handleFilter} disabled={isLoading}>
