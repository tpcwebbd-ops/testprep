...
type DisplayablePaymentsKeys = 
    ...
    | 'verified_by_id'
    | 'createdAt'
type ColumnVisibilityState = Record<DisplayablePaymentsKeys, boolean>


const ViewTableNextComponents: React.FC = () => {
    const [sortConfig, setSortConfig] = useState<{
        key: DisplayablePaymentsKeys
        direction: 'asc' | 'desc'
    } | null>(null)
    ...
    const allData = useMemo(
        () => getResponseData?.data?.payments || [],
        [getResponseData]
    )

    const tableHeaders: { key: DisplayablePaymentsKeys; label: string }[] = [
        { key: 'payer_name', label: 'Payer_name' },
        { key: 'payer_email', label: 'Payer_email' },
        { key: 'user_id', label: 'User_id' },
        { key: 'amount', label: 'Amount' },
        { key: 'transaction_id', label: 'Transaction_id' },
        { key: 'payment_method', label: 'Payment_method' },
        { key: 'verified_by_id', label: 'Verified_by_id' },
    ...
    const handleSort = (key: DisplayablePaymentsKeys) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: IPayments) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const renderActions = (item: IPayments) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedPayments(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedPayments(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedPayments(item); toggleDeleteModal(true); }}>
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <ExportDialog
                ...
                data={bulkData}
                fileName={`Exported_Payments_${new Date().toISOString()}.xlsx`}
            />
        </div>
    )
}
