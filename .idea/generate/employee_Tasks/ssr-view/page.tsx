...
const Page = async () => {
    const fetchData = async () => {
        ...
        const url =
            'http://localhost:3000/generate/employee_Tasks/all/api/v1?page=1&limit=4'
        ...
        try {
            ...
            return responseData.data?.employee_Tasks
        } catch (error) {
            ...
            {data &&
                ...
                data.map((i: { title: string; _id: string }, idx: number) => (
                    <div key={idx + i?.title}>
                        <CustomLInk
                            name={i.title}
                            url={`/generate/employee_Tasks/ssr-view/details/${i._id}`}
                        />
                    </div>
                ))}
        </main>
