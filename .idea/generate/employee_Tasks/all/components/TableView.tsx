...
type DisplayableEmployee_TasksKeys = 
    ...
    | 'end_date'
    | 'createdAt'
type ColumnVisibilityState = Record<DisplayableEmployee_TasksKeys, boolean>


const ViewTableNextComponents: React.FC = () => {
    const [sortConfig, setSortConfig] = useState<{
        key: DisplayableEmployee_TasksKeys
        direction: 'asc' | 'desc'
    } | null>(null)
    ...
    const allData = useMemo(
        () => getResponseData?.data?.employee_Tasks || [],
        [getResponseData]
    )

    const tableHeaders: { key: DisplayableEmployee_TasksKeys; label: string }[] = [
        { key: 'title', label: 'Title' },
        { key: 'assigned_to_id', label: 'Assigned_to_id' },
        { key: 'assigned_by_id', label: 'Assigned_by_id' },
    ...
    const handleSort = (key: DisplayableEmployee_TasksKeys) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: IEmployee_Tasks) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const renderActions = (item: IEmployee_Tasks) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedEmployee_Tasks(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedEmployee_Tasks(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedEmployee_Tasks(item); toggleDeleteModal(true); }}>
    ...
    const renderTableRows = () =>
        sortedData.map((item: IEmployee_Tasks) => (
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <ExportDialog
                ...
                data={bulkData}
                fileName={`Exported_Employee_Tasks_${new Date().toISOString()}.xlsx`}
            />
        </div>
    )
}
