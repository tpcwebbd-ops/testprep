...
const Page = async () => {
    const fetchData = async () => {
        ...
        const url =
            'http://localhost:3000/generate/website_Settings/all/api/v1?page=1&limit=4'
        ...
        try {
            ...
            return responseData.data?.website_Settings
        } catch (error) {
            ...
            {data &&
                ...
                data.map((i: { Name: string; _id: string }, idx: number) => (
                    <div key={idx + i?.Name}>
                        <CustomLInk
                            name={i.Name}
                            url={`/generate/website_Settings/ssr-view/details/${i._id}`}
                        />
                    </div>
                ))}
        </main>
