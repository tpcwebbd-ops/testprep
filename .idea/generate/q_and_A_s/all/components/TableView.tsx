...
type DisplayableQ_and_A_sKeys = 
    ...
    | 'answer_time'
    | 'createdAt'
type ColumnVisibilityState = Record<DisplayableQ_and_A_sKeys, boolean>


const ViewTableNextComponents: React.FC = () => {
    const [sortConfig, setSortConfig] = useState<{
        key: DisplayableQ_and_A_sKeys
        direction: 'asc' | 'desc'
    } | null>(null)
    ...
    const allData = useMemo(
        () => getResponseData?.data?.q_and_A_s || [],
        [getResponseData]
    )

    const tableHeaders: { key: DisplayableQ_and_A_sKeys; label: string }[] = [
        { key: 'course_id', label: 'Course_id' },
        { key: 'lecture_id', label: 'Lecture_id' },
        { key: 'student_uid', label: 'Student_uid' },
        { key: 'question_time', label: 'Question_time' },
        { key: 'answered_by_user_id', label: 'Answered_by_user_id' },
        { key: 'answer_time', label: 'Answer_time' },
    ...
    const handleSort = (key: DisplayableQ_and_A_sKeys) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: IQ_and_A_s) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const renderActions = (item: IQ_and_A_s) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedQ_and_A_s(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedQ_and_A_s(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedQ_and_A_s(item); toggleDeleteModal(true); }}>
    ...
    const renderTableRows = () =>
        sortedData.map((item: IQ_and_A_s) => (
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <ExportDialog
                ...
                data={bulkData}
                fileName={`Exported_Q_and_A_s_${new Date().toISOString()}.xlsx`}
            />
        </div>
    )
}
