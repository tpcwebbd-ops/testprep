...
import { useBulkUpdateCompany_GoalsMutation } from '../redux/rtk-api'
...
const BulkDynamicUpdateNextComponents: React.FC = () => {
    ...
    const [bulkUpdateCompany_Goals, { isLoading }] = useBulkUpdateCompany_GoalsMutation()

    const handleBulkEditCompany_Goals = async () => {
        ...
        try {
            ...
            await bulkUpdateCompany_Goals(newBulkData).unwrap()
            toggleBulkDynamicUpdateModal(false)
            setBulkData([])
            setNewItemTags([])
            handleSuccess('Update Successful')
        } catch (error) {
            console.error('Failed to edit company_Goals:', error)
            ...
            <DialogContent>
                ...
                <ScrollArea className="h-[400px] w-full rounded-md border p-4">
                    <div className="flex flex-col gap-2">
                        {bulkData.map((company_goal, idx) => (
                            <div
                                key={(company_goal._id as string) || idx}
                                className="flex items-start mb-2 justify-between flex-col"
                            >
                                <div className="flex flex-col">
                                    <span>
                                        {idx + 1}.{' '}
                                        {(company_goal)['title'] as string || ''}
                                    </span>
                ...
                <DialogFooter>
                    ...
                    <Button
                        disabled={isLoading}
                        variant="outline"
                        onClick={handleBulkEditCompany_Goals}
