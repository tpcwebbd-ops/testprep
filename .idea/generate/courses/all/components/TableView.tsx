...
import { ICourses } from '../store/data/data'
...
type DisplayableCoursesKeys = 
    | 'title'
    | 'duration_months'
    | 'total_lectures'
    | 'createdAt'
type ColumnVisibilityState = Record<DisplayableCoursesKeys, boolean>


const ViewTableNextComponents: React.FC = () => {
    const [sortConfig, setSortConfig] = useState<{
        key: DisplayableCoursesKeys
        direction: 'asc' | 'desc'
    } | null>(null)
    ...
    const allData = useMemo(
        () => getResponseData?.data?.courses || [],
        [getResponseData]
    )

    const tableHeaders: { key: DisplayableCoursesKeys; label: string }[] = [
        { key: 'title', label: 'Title' },
        { key: 'duration_months', label: 'Duration_months' },
        { key: 'total_lectures', label: 'Total_lectures' },
    ...
    const handleSort = (key: DisplayableCoursesKeys) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: ICourses) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const renderActions = (item: ICourses) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedCourses(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedCourses(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedCourses(item); toggleDeleteModal(true); }}>
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <ExportDialog
                ...
                data={bulkData}
                fileName={`Exported_Courses_${new Date().toISOString()}.xlsx`}
            />
        </div>
    )
}
