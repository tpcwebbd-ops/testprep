...
type DisplayableSubmissionsKeys = 
    ...
    | 'createdAt'
type ColumnVisibilityState = Record<DisplayableSubmissionsKeys, boolean>


const ViewTableNextComponents: React.FC = () => {
    const [sortConfig, setSortConfig] = useState<{
        key: DisplayableSubmissionsKeys
        direction: 'asc' | 'desc'
    } | null>(null)
    ...
    const allData = useMemo(
        () => getResponseData?.data?.submissions || [],
        [getResponseData]
    )

    const tableHeaders: { key: DisplayableSubmissionsKeys; label: string }[] = [
        ...
        { key: 'checked_by_mentor_id', label: 'Checked_by_mentor_id' },
    ...
    const handleSort = (key: DisplayableSubmissionsKeys) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: ISubmissions) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const renderActions = (item: ISubmissions) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedSubmissions(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedSubmissions(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedSubmissions(item); toggleDeleteModal(true); }}>
    ...
    const renderTableRows = () =>
        sortedData.map((item: ISubmissions) => (
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <ExportDialog
                ...
                data={bulkData}
                fileName={`Exported_Submissions_${new Date().toISOString()}.xlsx`}
            />
        </div>
    )
}
