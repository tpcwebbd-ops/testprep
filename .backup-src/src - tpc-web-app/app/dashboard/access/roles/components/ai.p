...
here is expected output for updateData. 
"""
{
    "name": "",
...
1. You have to update """  {Object.keys(defaultCURD).map(key => (
                              <Checkbox key={key} title={key} checked={false} onCheckedChange={() => ''} />
                            ))}"""
                          
2. So it can handle userAccess. 

Note: Do not edit style or color combination. 