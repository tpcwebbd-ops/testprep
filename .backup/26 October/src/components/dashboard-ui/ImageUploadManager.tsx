...
const InternalImageDialog = ({ onImageSelect, selectedImages }: InternalImageDialogProps) => {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch(`https://api.imgbb.com/1/upload?key=${process.env.NEXT_PUBLIC_IMGBB_API_KEY}`, { method: 'POST', body: formData });
  ...
  return (
    <ScrollArea className="w-full h-[60vh] p-4">
      ...
      <main>
        {isLoading ? (
          <div className="flex justify-center items-center h-48">Loading images...</div>
          ...
          <div className="text-center text-gray-50 py-10">No images found in the library.</div>
        )}
      </main>
    </ScrollArea>
  );
...
export default function ImageUploadManager({ value, onChange, label = 'Images' }: ImageUploadManagerProps) {
      ...
      <div className="w-full min-h-[120px] rounded-lg flex items-center justify-center p-2 border-2 border-dashed border-gray-300">
        {value.length > 0 ? (
          <div className="w-full grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            {value.map(imageUrl => (
              <div key={imageUrl} className="relative w-full aspect-square rounded-lg overflow-hidden shadow-md group">
