...
import { useBulkDeleteBatchesMutation } from '../redux/rtk-api'
...
const BulkDeleteNextComponents: React.FC = () => {
    ...
    const [bulkDeleteBatches, { isLoading }] = useBulkDeleteBatchesMutation()

    const handleBulkDelete = async () => {
        if (!bulkData?.length) return
        try {
            const ids = bulkData.map((batches) => batches._id)
            await bulkDeleteBatches({ ids }).unwrap()
            ...
            <DialogContent>
                ...
                <ScrollArea className="h-[400px] w-full rounded-md border p-4">
                    <div className="flex flex-col">
                        {bulkData.map((batches, idx) => (
                            <span
                                key={(batches._id as string) + idx}
                                className="text-xs"
                            >
                                {idx + 1}. {(batches)['batch_title'] as string || ''}
                            </span>
                        ))}
                    </div>
                </ScrollArea>
