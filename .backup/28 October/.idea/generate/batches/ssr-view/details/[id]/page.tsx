...
const DataDetails = ({ data }: { data: IBatch }) => {
    return (
        <div className="w-full flex flex-col md:p-4 p-1 gap-4">
            ...
            <div className="border border-slate-400 rounded-md overflow-hidden">
                ...
                <strong className="capitalize">batch_title:</strong> {data?.["batch_title"]?.toString()}
            </div>
            <div className="w-full hover:bg-slate-400 bg-slate-300 block p-2 border-b border-slate-400">
            ...
            <div className="w-full hover:bg-slate-400 bg-slate-300 block p-2 border-b border-slate-400">
                <strong className="capitalize">instructors_user_id:</strong> {data?.["instructors_user_id"]?.toString()}
            </div>
            <div className="w-full hover:bg-slate-400 bg-slate-300 block p-2 border-b border-slate-400">
                <strong className="capitalize">instructors_name:</strong> {data?.["instructors_name"]?.toString()}
            </div>
            <div className="w-full hover:bg-slate-400 bg-slate-300 block p-2 border-b border-slate-400">
                <strong className="capitalize">enroll_students:</strong> {data?.["enroll_students"]?.toString()}
            </div>
            </div>
            <HomeButton />
        </div>
...
const getDataById = async (id: string): Promise<ApiResponse> => {
    const backendUrl = `http://localhost:3000/generate/batches/all/api/v1?id=${id}`
...
export async function generateMetadata({ params }: { params: { id: string } }) {
    ...
    return {
        title: data?.data?.["batch_title"]?.toString() || 'Batch',
    }
}
