...
type DisplayableUsersKeys = 
    | 'full_name'
    | 'uid'
    | 'email'
    | 'password_hash'
    | 'createdAt'
type ColumnVisibilityState = Record<DisplayableUsersKeys, boolean>


const ViewTableNextComponents: React.FC = () => {
    const [sortConfig, setSortConfig] = useState<{
        key: DisplayableUsersKeys
        direction: 'asc' | 'desc'
    } | null>(null)
    ...
    const allData = useMemo(
        () => getResponseData?.data?.users || [],
        [getResponseData]
    )

    const tableHeaders: { key: DisplayableUsersKeys; label: string }[] = [
    ...
    const handleSort = (key: DisplayableUsersKeys) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: IUsers) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const renderActions = (item: IUsers) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedUsers(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedUsers(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedUsers(item); toggleDeleteModal(true); }}>
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <ExportDialog
                ...
                data={bulkData}
                fileName={`Exported_Users_${new Date().toISOString()}.xlsx`}
            />
        </div>
    )
}
