...
const ViewTableNextComponents: React.FC = () => {
    ...
    const allData = useMemo(
        () => getResponseData?.data?.website_Settings || [],
        [getResponseData]
    )

    const tableHeaders: { key: DisplayableWebsite_SettingsKeys; label: string }[] = [
        { key: 'Name', label: ' Name' },
        { key: 'menu', label: 'Menu' },
    ...
    const handleSort = (key: DisplayableWebsite_SettingsKeys) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: IWebsite_Settings) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const renderActions = (item: IWebsite_Settings) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedWebsite_Settings(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedWebsite_Settings(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedWebsite_Settings(item); toggleDeleteModal(true); }}>
    ...
    const renderTableRows = () =>
        sortedData.map((item: IWebsite_Settings) => (
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <ExportDialog
                ...
                data={bulkData}
                fileName={`Exported_Website_Settings_${new Date().toISOString()}.xlsx`}
            />
        </div>
    )
}
