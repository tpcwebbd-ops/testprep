...
import { website_SettingsSelectorArr } from '../store/store-constant'
import { useBulkUpdateWebsite_SettingsMutation } from '../redux/rtk-api'
...
const BulkUpdateNextComponents: React.FC = () => {
    ...
    const [bulkUpdateWebsite_Settings, { isLoading }] = useBulkUpdateWebsite_SettingsMutation()
    ...
    const handleBulkUpdate = async () => {
        ...
        try {
            ...
            await bulkUpdateWebsite_Settings(newBulkData).unwrap()
            toggleBulkUpdateModal(false)
            setBulkData([])
            handleSuccess('Update Successful')
        } catch (error) {
            console.error('Failed to edit website_Settings:', error)
    ...
    const handleFieldChangeForAll = (value: string) => {
        setBulkData(
            bulkData.map((website_setting) => ({
                ...website_setting,
                ['role']: value,
            })) as IWebsite_Settings[]
        )
    }
            ...
            <DialogContent>
                    ...
                    <div>
                        ...
                        <div className="w-full flex items-center justify-between pt-2">
                            ...
                            <Select
                                ...
                                defaultValue={
                                    (website_SettingsSelectorArr[0] as string) || ''
                                }
                            >
                                ...
                                <SelectContent className="bg-slate-50">
                                    {website_SettingsSelectorArr?.map((option, index) => (
                                        <SelectItem
                                            key={option + index}
                                            value={option}
                ...
                <ScrollArea className="h-[400px] w-full rounded-md border p-4">
                    <div className="flex flex-col gap-2">
                        {bulkData.map((website_setting, idx) => (
                            <div
                                key={(website_setting._id as string) || idx}
                                className="flex items-center justify-between"
                            >
                                <span>
                                    {idx + 1}. {(website_setting)['Name'] as string || ''}
                                </span>
                                <span className="text-blue-500">{website_setting['Name'] as string}</span>
                            </div>
                        ))}
                    </div>
                </ScrollArea>
