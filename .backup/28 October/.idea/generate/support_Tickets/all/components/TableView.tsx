...
type DisplayableSupport_TicketsKeys = 
    ...
    | 'closed_at'
    | 'createdAt'
type ColumnVisibilityState = Record<DisplayableSupport_TicketsKeys, boolean>


const ViewTableNextComponents: React.FC = () => {
    const [sortConfig, setSortConfig] = useState<{
        key: DisplayableSupport_TicketsKeys
        direction: 'asc' | 'desc'
    } | null>(null)
    ...
    const allData = useMemo(
        () => getResponseData?.data?.support_Tickets || [],
        [getResponseData]
    )

    const tableHeaders: { key: DisplayableSupport_TicketsKeys; label: string }[] = [
    ...
    const handleSort = (key: DisplayableSupport_TicketsKeys) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: ISupport_Tickets) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const renderActions = (item: ISupport_Tickets) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedSupport_Tickets(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedSupport_Tickets(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedSupport_Tickets(item); toggleDeleteModal(true); }}>
    ...
    const renderTableRows = () =>
        sortedData.map((item: ISupport_Tickets) => (
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <ExportDialog
                ...
                data={bulkData}
                fileName={`Exported_Support_Tickets_${new Date().toISOString()}.xlsx`}
            />
        </div>
    )
}
