...
const ViewTableNextComponents: React.FC = () => {
    ...
    const allData = useMemo(
        () => getResponseData?.data?.marketing_Leads || [],
        [getResponseData]
    )

    const tableHeaders: { key: DisplayableMarketing_LeadsKeys; label: string }[] = [
        { key: 'full_name', label: 'Full_name' },
        { key: 'email', label: 'Email' },
        { key: 'collected_by', label: 'Collected_by' },
    ...
    const handleSort = (key: DisplayableMarketing_LeadsKeys) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: IMarketing_Leads) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const renderActions = (item: IMarketing_Leads) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedMarketing_Leads(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedMarketing_Leads(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedMarketing_Leads(item); toggleDeleteModal(true); }}>
    ...
    const renderTableRows = () =>
        sortedData.map((item: IMarketing_Leads) => (
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <ExportDialog
                ...
                data={bulkData}
                fileName={`Exported_Marketing_Leads_${new Date().toISOString()}.xlsx`}
            />
        </div>
    )
}
