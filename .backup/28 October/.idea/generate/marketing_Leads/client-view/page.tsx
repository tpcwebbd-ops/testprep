...
const Page = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const url =
                'http://localhost:3000/dashboard/marketing_Lead/all/api/v1?page=1&limit=4'
            ...
            try {
                ...
                setData(responseData?.data?.marketing_Leads || [])
            } catch (error) {
        ...
        <main className="w-full flex flex-col gap-2 p-1 md:p-4">
            {data?.map((item: DataItem, idx: number) => (
                <div key={idx + item?._id}>
                    <CustomLInk
                        name={item.full_name}
                        url={`/dashboard/marketing_Lead/client-view/details/${item._id}`}
                    />
                </div>
            ))}
        </main>
