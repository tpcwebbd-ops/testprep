...
// CREATE Attendance
export async function createAttendance(req: Request): Promise<IResponse> {
    return withDB(async () => {
        try {
...
export async function getAttendanceById(req: Request): Promise<IResponse> {
    return withDB(async () => {
        const id = new URL(req.url).searchParams.get('id')
        if (!id)
            return formatResponse(null, 'Attendance ID is required', 400)

        const attendance = await Attendance.findById(id)
        if (!attendance)
            return formatResponse(null, 'Attendance not found', 404)
...
// GET all Attendance_s with pagination and intelligent search
export async function getAttendance_s(req: Request): Promise<IResponse> {
    return withDB(async () => {
        ...
        if (searchQuery) {
            ...
            if (searchQuery.startsWith('createdAt:range:')) {
                ...
                const stringFields = ["user_id","notes"];
        ...
        const attendance_s = await Attendance.find(searchFilter)
            .sort({ updatedAt: -1, createdAt: -1 })
            .skip(skip)
            .limit(limit)

        const totalAttendance_s =
            await Attendance.countDocuments(searchFilter)

        return formatResponse(
            {
                attendance_s: attendance_s || [],
                total: totalAttendance_s,
                page,
                limit,
            },
            'Attendance_s fetched successfully',
            200
        )
    })
}
...
export async function updateAttendance(req: Request): Promise<IResponse> {
    return withDB(async () => {
        try {
...
// BULK UPDATE Attendance_s
export async function bulkUpdateAttendance_s(req: Request): Promise<IResponse> {
    return withDB(async () => {
        const updates: { id: string; updateData: Record<string, unknown> }[] = await req.json()
        const results = await Promise.allSettled(
            updates.map(({ id, updateData }) =>
                Attendance.findByIdAndUpdate(id, updateData, {
                    new: true,
                    runValidators: true,
                })
            )
...
export async function deleteAttendance(req: Request): Promise<IResponse> {
    return withDB(async () => {
...
// BULK DELETE Attendance_s
export async function bulkDeleteAttendance_s(req: Request): Promise<IResponse> {
    return withDB(async () => {
        ...
        for (const id of ids) {
            try {
                ...
                if (doc) {
                    const deletedDoc = await Attendance.findByIdAndDelete(id)
                    if (deletedDoc) {
                        deletedIds.push(id)
                    }
                } else {
