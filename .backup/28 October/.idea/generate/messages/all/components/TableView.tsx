...
type DisplayableMessagesKeys = 
    ...
    | 'replayDate'
    | 'createdAt'
type ColumnVisibilityState = Record<DisplayableMessagesKeys, boolean>


const ViewTableNextComponents: React.FC = () => {
    const [sortConfig, setSortConfig] = useState<{
        key: DisplayableMessagesKeys
        direction: 'asc' | 'desc'
    } | null>(null)
    ...
    const allData = useMemo(
        () => getResponseData?.data?.messages || [],
        [getResponseData]
    )

    const tableHeaders: { key: DisplayableMessagesKeys; label: string }[] = [
        { key: 'sender_Name', label: 'Sender_ Name' },
        { key: 'sent_time', label: 'Sent_time' },
        { key: 'sent_date', label: 'Sent_date' },
        { key: 'replayBy', label: 'Replay By' },
        { key: 'replayTime', label: 'Replay Time' },
        { key: 'replayDate', label: 'Replay Date' },
    ...
    const handleSort = (key: DisplayableMessagesKeys) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: IMessages) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const renderActions = (item: IMessages) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedMessages(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedMessages(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedMessages(item); toggleDeleteModal(true); }}>
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <ExportDialog
                ...
                data={bulkData}
                fileName={`Exported_Messages_${new Date().toISOString()}.xlsx`}
            />
        </div>
    )
}
