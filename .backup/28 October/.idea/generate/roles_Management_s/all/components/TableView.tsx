...
import { useGetRoles_Management_sQuery } from '../redux/rtk-api'
import Pagination from './Pagination'
import ExportDialog from './ExportDialog' // Import the new dialog component

// Dynamically generated types for type safety
type DisplayableRoles_Management_sKeys = 
    | 'users_id'
    | 'role_ids'
    | 'createdAt'
type ColumnVisibilityState = Record<DisplayableRoles_Management_sKeys, boolean>


const ViewTableNextComponents: React.FC = () => {
    const [sortConfig, setSortConfig] = useState<{
        key: DisplayableRoles_Management_sKeys
        direction: 'asc' | 'desc'
    } | null>(null)
    ...
    const allData = useMemo(
        () => getResponseData?.data?.roles_Management_s || [],
        [getResponseData]
    )

    const tableHeaders: { key: DisplayableRoles_Management_sKeys; label: string }[] = [
        { key: 'users_id', label: 'Users_id' },
        { key: 'role_ids', label: 'Role_ids' },
    ...
    const handleSort = (key: DisplayableRoles_Management_sKeys) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: IRoles_Management_s) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const renderActions = (item: IRoles_Management_s) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedRoles_Management_s(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedRoles_Management_s(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedRoles_Management_s(item); toggleDeleteModal(true); }}>
    ...
    const renderTableRows = () =>
        sortedData.map((item: IRoles_Management_s) => (
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <ExportDialog
                ...
                data={bulkData}
                fileName={`Exported_Roles_Management_s_${new Date().toISOString()}.xlsx`}
            />
        </div>
    )
}
