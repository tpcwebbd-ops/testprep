...
export async function getMediaById(req: Request): Promise<IResponse> {
    return withDB(async () => {
        ...
        if (!id)
            return formatResponse(null, 'Media ID is required', 400)

        const media = await Media.findById(id)
        if (!media)
            return formatResponse(null, 'Media not found', 404)
...
// GET all Media_s with pagination and intelligent search
export async function getMedia_s(req: Request): Promise<IResponse> {
    return withDB(async () => {
        ...
        if (searchQuery) {
            ...
            if (searchQuery.startsWith('createdAt:range:')) {
                ...
                const stringFields = ["uploader_email","file_name","file_url"];
        ...
        const media_s = await Media.find(searchFilter)
            .sort({ updatedAt: -1, createdAt: -1 })
            .skip(skip)
            .limit(limit)

        const totalMedia_s =
            await Media.countDocuments(searchFilter)
        ...
        return formatResponse(
            ...
            'Media_s fetched successfully',
            200
        )
    })
}

// UPDATE single Media by ID
...
// BULK UPDATE Media_s
export async function bulkUpdateMedia_s(req: Request): Promise<IResponse> {
    return withDB(async () => {
        const updates: { id: string; updateData: Record<string, unknown> }[] = await req.json()
        const results = await Promise.allSettled(
            updates.map(({ id, updateData }) =>
                Media.findByIdAndUpdate(id, updateData, {
                    new: true,
                    runValidators: true,
                })
            )
...
// BULK DELETE Media_s
export async function bulkDeleteMedia_s(req: Request): Promise<IResponse> {
    return withDB(async () => {
        ...
        for (const id of ids) {
            try {
                ...
                if (doc) {
                    const deletedDoc = await Media.findByIdAndDelete(id)
                    if (deletedDoc) {
                        deletedIds.push(id)
                    }
                } else {
