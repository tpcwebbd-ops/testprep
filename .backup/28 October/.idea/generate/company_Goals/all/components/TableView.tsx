...
type DisplayableCompany_GoalsKeys = 
    ...
    | 'checked_by_user_id'
    | 'createdAt'
type ColumnVisibilityState = Record<DisplayableCompany_GoalsKeys, boolean>


const ViewTableNextComponents: React.FC = () => {
    const [sortConfig, setSortConfig] = useState<{
        key: DisplayableCompany_GoalsKeys
        direction: 'asc' | 'desc'
    } | null>(null)
    ...
    const allData = useMemo(
        () => getResponseData?.data?.company_Goals || [],
        [getResponseData]
    )

    const tableHeaders: { key: DisplayableCompany_GoalsKeys; label: string }[] = [
        { key: 'title', label: 'Title' },
        { key: 'set_by_user_id', label: 'Set_by_user_id' },
    ...
    const handleSort = (key: DisplayableCompany_GoalsKeys) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: ICompany_Goals) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const renderActions = (item: ICompany_Goals) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedCompany_Goals(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedCompany_Goals(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedCompany_Goals(item); toggleDeleteModal(true); }}>
    ...
    const renderTableRows = () =>
        sortedData.map((item: ICompany_Goals) => (
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <ExportDialog
                ...
                data={bulkData}
                fileName={`Exported_Company_Goals_${new Date().toISOString()}.xlsx`}
            />
        </div>
    )
}
