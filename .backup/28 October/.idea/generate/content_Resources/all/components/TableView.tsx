...
const ViewTableNextComponents: React.FC = () => {
    ...
    const allData = useMemo(
        () => getResponseData?.data?.content_Resources || [],
        [getResponseData]
    )

    const tableHeaders: { key: DisplayableContent_ResourcesKeys; label: string }[] = [
        { key: 'lecture_id', label: 'Lecture_id' },
    ...
    const handleSort = (key: DisplayableContent_ResourcesKeys) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: IContent_Resources) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const renderActions = (item: IContent_Resources) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedContent_Resources(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedContent_Resources(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedContent_Resources(item); toggleDeleteModal(true); }}>
    ...
    const renderTableRows = () =>
        sortedData.map((item: IContent_Resources) => (
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <ExportDialog
                ...
                data={bulkData}
                fileName={`Exported_Content_Resources_${new Date().toISOString()}.xlsx`}
            />
        </div>
    )
}
