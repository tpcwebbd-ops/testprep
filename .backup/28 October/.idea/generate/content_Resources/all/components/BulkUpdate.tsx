...
import { content_ResourcesSelectorArr } from '../store/store-constant'
import { useBulkUpdateContent_ResourcesMutation } from '../redux/rtk-api'
...
const BulkUpdateNextComponents: React.FC = () => {
    ...
    const [bulkUpdateContent_Resources, { isLoading }] = useBulkUpdateContent_ResourcesMutation()
    ...
    const handleBulkUpdate = async () => {
        ...
        try {
            ...
            await bulkUpdateContent_Resources(newBulkData).unwrap()
            toggleBulkUpdateModal(false)
            setBulkData([])
            handleSuccess('Update Successful')
        } catch (error) {
            console.error('Failed to edit content_Resources:', error)
    ...
    const handleFieldChangeForAll = (value: string) => {
        setBulkData(
            bulkData.map((content_resourc) => ({
                ...content_resourc,
                ['role']: value,
            })) as IContent_Resources[]
        )
    }
            ...
            <DialogContent>
                    ...
                    <div>
                        ...
                        <div className="w-full flex items-center justify-between pt-2">
                            ...
                            <Select
                                ...
                                defaultValue={
                                    (content_ResourcesSelectorArr[0] as string) || ''
                                }
                            >
                                ...
                                <SelectContent className="bg-slate-50">
                                    {content_ResourcesSelectorArr?.map((option, index) => (
                                        <SelectItem
                                            key={option + index}
                                            value={option}
                ...
                <ScrollArea className="h-[400px] w-full rounded-md border p-4">
                    <div className="flex flex-col gap-2">
                        {bulkData.map((content_resourc, idx) => (
                            <div
                                key={(content_resourc._id as string) || idx}
                                className="flex items-center justify-between"
                            >
                                <span>
                                    {idx + 1}. {(content_resourc)['title'] as string || ''}
                                </span>
                                <span className="text-blue-500">{content_resourc['title'] as string}</span>
                            </div>
                        ))}
                    </div>
                </ScrollArea>
