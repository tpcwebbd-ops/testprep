...
const Page = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const url =
                'http://localhost:3000/dashboard/content_Resourc/all/api/v1?page=1&limit=4'
            ...
            try {
                ...
                setData(responseData?.data?.content_Resources || [])
            } catch (error) {
        ...
        <main className="w-full flex flex-col gap-2 p-1 md:p-4">
            {data?.map((item: DataItem, idx: number) => (
                <div key={idx + item?._id}>
                    <CustomLInk
                        name={item.title}
                        url={`/dashboard/content_Resourc/client-view/details/${item._id}`}
                    />
                </div>
            ))}
        </main>
