...
interface ApiResponse {
    data: IContent_Resourc;
    message: string;
    status: number;
}

const DataDetails = ({ data }: { data: IContent_Resourc }) => {
    return (
        <div className="w-full flex flex-col md:p-4 p-1 gap-4">
            <h1 className="text-2xl font-bold">Content_Resourc Details</h1>
...
const getDataById = async (id: string): Promise<ApiResponse> => {
    const backendUrl = `http://localhost:3000/generate/content_Resources/all/api/v1?id=${id}`
    ...
    try {
        ...
        console.error('Failed to fetch Content_Resourc:', error)
        notFound();
    }
}
...
export async function generateMetadata({ params }: { params: { id: string } }) {
    ...
    return {
        title: data?.data?.["title"]?.toString() || 'Content_Resourc',
    }
}
