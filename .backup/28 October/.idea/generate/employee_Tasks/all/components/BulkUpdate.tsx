...
import { employee_TasksSelectorArr } from '../store/store-constant'
import { useBulkUpdateEmployee_TasksMutation } from '../redux/rtk-api'
...
const BulkUpdateNextComponents: React.FC = () => {
    ...
    const [bulkUpdateEmployee_Tasks, { isLoading }] = useBulkUpdateEmployee_TasksMutation()
    ...
    const handleBulkUpdate = async () => {
        ...
        try {
            ...
            await bulkUpdateEmployee_Tasks(newBulkData).unwrap()
            toggleBulkUpdateModal(false)
            setBulkData([])
            handleSuccess('Update Successful')
        } catch (error) {
            console.error('Failed to edit employee_Tasks:', error)
            ...
            <DialogContent>
                    ...
                    <div>
                        ...
                        <div className="w-full flex items-center justify-between pt-2">
                            ...
                            <Select
                                ...
                                defaultValue={
                                    (employee_TasksSelectorArr[0] as string) || ''
                                }
                            >
                                ...
                                <SelectContent className="bg-slate-50">
                                    {employee_TasksSelectorArr?.map((option, index) => (
                                        <SelectItem
                                            key={option + index}
                                            value={option}
                ...
                <ScrollArea className="h-[400px] w-full rounded-md border p-4">
                    <div className="flex flex-col gap-2">
                        {bulkData.map((employee_task, idx) => (
                            <div
                                key={(employee_task._id as string) || idx}
                                className="flex items-center justify-between"
                            >
                                <span>
                                    {idx + 1}. {(employee_task)['title'] as string || ''}
                                </span>
                                <span className="text-blue-500">{employee_task['title'] as string}</span>
                            </div>
                        ))}
                    </div>
                </ScrollArea>
