...
type DisplayableEnrollmentsKeys = 
    ...
    | 'payment_id'
    | 'createdAt'
type ColumnVisibilityState = Record<DisplayableEnrollmentsKeys, boolean>


const ViewTableNextComponents: React.FC = () => {
    const [sortConfig, setSortConfig] = useState<{
        key: DisplayableEnrollmentsKeys
        direction: 'asc' | 'desc'
    } | null>(null)
    ...
    const allData = useMemo(
        () => getResponseData?.data?.enrollments || [],
        [getResponseData]
    )

    const tableHeaders: { key: DisplayableEnrollmentsKeys; label: string }[] = [
        { key: 'student_email  ', label: 'Student_email  ' },
        { key: 'batch_id', label: 'Batch_id' },
        { key: 'enrollment_date', label: 'Enrollment_date' },
        { key: 'enrollment_time', label: 'Enrollment_time' },
    ...
    const handleSort = (key: DisplayableEnrollmentsKeys) => {
        setSortConfig((prev) =>
            prev?.key === key
    ...
    const handleSelectRow = (isChecked: boolean, item: IEnrollments) =>
        setBulkData(
            isChecked
                ? [...bulkData, item]
    ...
    const renderActions = (item: IEnrollments) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedEnrollments(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedEnrollments(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedEnrollments(item); toggleDeleteModal(true); }}>
    ...
    const renderTableRows = () =>
        sortedData.map((item: IEnrollments) => (
    ...
    return (
        <div className="w-full flex flex-col">
            ...
            <ExportDialog
                ...
                data={bulkData}
                fileName={`Exported_Enrollments_${new Date().toISOString()}.xlsx`}
            />
        </div>
    )
}
