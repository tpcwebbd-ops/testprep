...
import { enrollmentsSelectorArr } from '../store/store-constant'
import { useBulkUpdateEnrollmentsMutation } from '../redux/rtk-api'
...
const BulkUpdateNextComponents: React.FC = () => {
    ...
    const [bulkUpdateEnrollments, { isLoading }] = useBulkUpdateEnrollmentsMutation()
    ...
    const handleBulkUpdate = async () => {
        ...
        try {
            ...
            await bulkUpdateEnrollments(newBulkData).unwrap()
            toggleBulkUpdateModal(false)
            setBulkData([])
            handleSuccess('Update Successful')
        } catch (error) {
            console.error('Failed to edit enrollments:', error)
            ...
            <DialogContent>
                    ...
                    <div>
                        ...
                        <div className="w-full flex items-center justify-between pt-2">
                            ...
                            <Select
                                ...
                                defaultValue={
                                    (enrollmentsSelectorArr[0] as string) || ''
                                }
                            >
                                ...
                                <SelectContent className="bg-slate-50">
                                    {enrollmentsSelectorArr?.map((option, index) => (
                                        <SelectItem
                                            key={option + index}
                                            value={option}
                ...
                <ScrollArea className="h-[400px] w-full rounded-md border p-4">
                    <div className="flex flex-col gap-2">
                        {bulkData.map((enrollment, idx) => (
                            <div
                                key={(enrollment._id as string) || idx}
                                className="flex items-center justify-between"
                            >
                                <span>
                                    {idx + 1}. {(enrollment)['batch_id'] as string || ''}
                                </span>
                                <span className="text-blue-500">{enrollment['batch_id'] as string}</span>
                            </div>
                        ))}
                    </div>
                </ScrollArea>
