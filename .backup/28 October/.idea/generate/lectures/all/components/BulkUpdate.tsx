...
import { lecturesSelectorArr } from '../store/store-constant'
import { useBulkUpdateLecturesMutation } from '../redux/rtk-api'
...
const BulkUpdateNextComponents: React.FC = () => {
    ...
    const [bulkUpdateLectures, { isLoading }] = useBulkUpdateLecturesMutation()
    ...
    const handleBulkUpdate = async () => {
        ...
        try {
            ...
            await bulkUpdateLectures(newBulkData).unwrap()
            toggleBulkUpdateModal(false)
            setBulkData([])
            handleSuccess('Update Successful')
        } catch (error) {
            console.error('Failed to edit lectures:', error)
            ...
            <DialogContent>
                    ...
                    <div>
                        ...
                        <div className="w-full flex items-center justify-between pt-2">
                            ...
                            <Select
                                ...
                                defaultValue={
                                    (lecturesSelectorArr[0] as string) || ''
                                }
                            >
                                ...
                                <SelectContent className="bg-slate-50">
                                    {lecturesSelectorArr?.map((option, index) => (
                                        <SelectItem
                                            key={option + index}
                                            value={option}
                ...
                <ScrollArea className="h-[400px] w-full rounded-md border p-4">
                    <div className="flex flex-col gap-2">
                        {bulkData.map((lecture, idx) => (
                            <div
                                key={(lecture._id as string) || idx}
                                className="flex items-center justify-between"
                            >
                                <span>
                                    {idx + 1}. {(lecture)['title'] as string || ''}
                                </span>
                                <span className="text-blue-500">{lecture['title'] as string}</span>
                            </div>
                        ))}
                    </div>
                </ScrollArea>
