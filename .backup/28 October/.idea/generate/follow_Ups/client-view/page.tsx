...
const Page = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const url =
                'http://localhost:3000/dashboard/follow_Up/all/api/v1?page=1&limit=4'
            ...
            try {
                ...
                setData(responseData?.data?.follow_Ups || [])
            } catch (error) {
        ...
        <main className="w-full flex flex-col gap-2 p-1 md:p-4">
            {data?.map((item: DataItem, idx: number) => (
                <div key={idx + item?._id}>
                    <CustomLInk
                        name={item.lead_id}
                        url={`/dashboard/follow_Up/client-view/details/${item._id}`}
                    />
                </div>
            ))}
        </main>
