...
import { follow_UpsSelectorArr } from '../store/store-constant'
import { useBulkUpdateFollow_UpsMutation } from '../redux/rtk-api'
...
const BulkUpdateNextComponents: React.FC = () => {
    ...
    const [bulkUpdateFollow_Ups, { isLoading }] = useBulkUpdateFollow_UpsMutation()
    ...
    const handleBulkUpdate = async () => {
        ...
        try {
            ...
            await bulkUpdateFollow_Ups(newBulkData).unwrap()
            toggleBulkUpdateModal(false)
            setBulkData([])
            handleSuccess('Update Successful')
        } catch (error) {
            console.error('Failed to edit follow_Ups:', error)
            ...
            <DialogContent>
                    ...
                    <div>
                        ...
                        <div className="w-full flex items-center justify-between pt-2">
                            ...
                            <Select
                                ...
                                defaultValue={
                                    (follow_UpsSelectorArr[0] as string) || ''
                                }
                            >
                                ...
                                <SelectContent className="bg-slate-50">
                                    {follow_UpsSelectorArr?.map((option, index) => (
                                        <SelectItem
                                            key={option + index}
                                            value={option}
                ...
                <ScrollArea className="h-[400px] w-full rounded-md border p-4">
                    <div className="flex flex-col gap-2">
                        {bulkData.map((follow_up, idx) => (
                            <div
                                key={(follow_up._id as string) || idx}
                                className="flex items-center justify-between"
                            >
                                <span>
                                    {idx + 1}. {(follow_up)['lead_id'] as string || ''}
                                </span>
                                <span className="text-blue-500">{follow_up['lead_id'] as string}</span>
                            </div>
                        ))}
                    </div>
                </ScrollArea>
