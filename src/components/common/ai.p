...
and here is updated brand-settings/model.ts 
```
...
and here is updated menu-editor/model.ts
```
import mongoose, { Schema, Document, Model } from 'mongoose';
...
I update brand-settings/model.ts and menu-editor/model.ts 

Now Your task is update MenuClient.tsx with the following instructions.
1. Make sure all implement 
    - menuTextColor: string;
    - menuFontSize: 'text-lg' | 'text-xl' | 'text-2xl' | 'text-3xl';
    - menuFontFamily: 'font-sans' | 'font-serif' | 'font-mono';
    - menuBackgroundColor: string;
2. do not use other color-combination and other style. 