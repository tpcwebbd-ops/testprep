/*
|-----------------------------------------
| setting up GeneratePersonalTableView for the App
| @author: Toufiquer Rahman<toufiquer.0@gmail.com>
| @copyright: Toufiquer, April, 2026
|-----------------------------------------
*/

export const generatePersonalViewTableComponentFile = (inputJsonFile: string): string => {
  ...
  const componentName = isPersonal ? `ViewPersonalTableNextComponents` : `ViewTableNextComponents`;
  ...
  const hookName = isPersonal ? `useGetPersonal${pluralPascalCase}Query` : `useGet${pluralPascalCase}Query`;

  let reduxPath = '';
  if (isPersonal) {
    reduxPath = `@/redux/features/${pluralLowerCase}/personal${pluralPascalCase}Slice`;
  } else if (isUsedGenerateFolder) {
    reduxPath = `../redux/rtk-api`;
  } else {
    reduxPath = `@/redux/features/${pluralLowerCase}/${pluralLowerCase}Slice`;
  }

  const queryParams = isPersonal
    ? `q: queryPramsQ,
  ...
  const rawHeaders = Object.entries(schema)
    .filter(
      ([key, type]) =>
        typeof type === 'string' && !key.includes('-') && suitableTypes.includes((type as string).toUpperCase()) && !excludedKeys.includes(key.toLowerCase()),
    )
    .slice(0, 7)
    .map(([key]) => ({
      origKey: key,
...
import { ${hookName} } from '${reduxPath}';
