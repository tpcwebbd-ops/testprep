/*
|-----------------------------------------
| setting up ImageUploadFieldSingle for the App
  ...
  return (
    <div className="flex flex-col items-center justify-center p-4 border rounded-lg shadow-sm w-full max-w-sm mx-auto">
      ...
      <div className="w-full">
        <Input type="file" accept="image/*" onChange={handleImageUpload} disabled={loading} className="mb-3" />
        {loading && (
        ...
        {!loading && !profileImageUrl && <p className="text-sm text-gray-500 mt-2 text-center">Upload your profile picture.</p>}
      </div>
    </div>
  );
};
