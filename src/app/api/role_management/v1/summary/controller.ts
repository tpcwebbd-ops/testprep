...
export async function getRoles_ManagementSummary(req: Request): Promise<IResponse> {
    return withDB(async () => {
        const now = new Date();
        const twentyFourHoursAgo = new Date(now.getTime() - 24 * 60 * 60 * 1000);

        const [totalDocs, last24HoursDocs] = await Promise.all([
            Roles_Management.countDocuments({}),
            Roles_Management.countDocuments({ createdAt: { $gte: twentyFourHoursAgo } }),
        ]);
