    ...
    return withDB(async () => {
        try {
            ...
            return formatResponse(
                newAccessManagement,
                'AccessManagement created successfully',
                201
            )
        } catch (error: unknown) {
...
export async function getAccessManagementById(req: Request): Promise<IResponse> {
    return withDB(async () => {
        ...
        if (!id)
            return formatResponse(null, 'AccessManagement ID is required', 400)

        const accessManagement = await AccessManagement.findById(id)
        if (!accessManagement)
            return formatResponse(null, 'AccessManagement not found', 404)

        return formatResponse(
            accessManagement,
            'AccessManagement fetched successfully',
            200
        )
    })
}
...
export async function getAccessManagements(req: Request): Promise<IResponse> {
    return withDB(async () => {
        ...
        const accessManagements = await AccessManagement.find(searchFilter)
            .sort({ updatedAt: -1, createdAt: -1 })
            .skip(skip)
            .limit(limit)

        const totalAccessManagements =
            await AccessManagement.countDocuments(searchFilter)
...
export async function updateAccessManagement(req: Request): Promise<IResponse> {
    return withDB(async () => {
        try {
            ...
            const updatedAccessManagement = await AccessManagement.findByIdAndUpdate(
                id,
                updateData,
                { new: true, runValidators: true }
            )

            if (!updatedAccessManagement)
                return formatResponse(null, 'AccessManagement not found', 404)
            return formatResponse(
                updatedAccessManagement,
                'AccessManagement updated successfully',
                200
            )
        } catch (error: unknown) {
            if ((error as { code?: number }).code === 11000) {
...
export async function deleteAccessManagement(req: Request): Promise<IResponse> {
    return withDB(async () => {
        ...
        if (!deletedAccessManagement)
            return formatResponse(
                null,
                'AccessManagement not found',
                404
            )
        return formatResponse(
            { deletedCount: 1 },
            'AccessManagement deleted successfully',
            200
        )
    })
}
