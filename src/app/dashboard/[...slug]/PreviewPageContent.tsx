...
import { PageContent } from '../db-builder/utils';
import { Allfields } from '../db-builder/all-fields/all-fields-index';
...
const normalizePathForMatch = (path: string) => path.replace(/[^a-z0-9]/gi, '').toLowerCase();
const getPathLastSegment = (path: string) => path.split('/').filter(Boolean).at(-1) || '';

interface PreviewPageContentProps {
  pathTitleOverride?: string;
  notFoundTitle?: string;
}

export function PreviewPageContent({ pathTitleOverride, notFoundTitle = 'Page Not Found' }: PreviewPageContentProps = {}) {
  const searchParams = useSearchParams();
  const pathTitle = pathTitleOverride || searchParams.get('pathTitle') || '/';
  ...
  const currentPage = useMemo(() => {
    const normalizedPathTitle = normalizePathForMatch(pathTitle);
    const lastSegment = getPathLastSegment(pathTitle);

    return (
      normalizedPages.find(p => p.path === pathTitle) ||
      normalizedPages.find(p => normalizePathForMatch(p.path) === normalizedPathTitle) ||
      normalizedPages.find(p => getPathLastSegment(p.path) === lastSegment)
    );
  ...
  const renderRecordForm = () => (
    <div className="grid gap-4 py-4 px-6 text-white">
      {fields.map(field => (
        <div
          key={field.id}
          className={`grid grid-cols-1 md:grid-cols-4 gap-4 pr-1 ${modalMode === 'view' ? 'items-center border-b border-white/10 py-2' : 'items-center'}`}
        >
  ...
  if (!currentPage) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-transparent text-white">
        <h2 className="text-2xl font-bold mb-2">{notFoundTitle}</h2>
    ...
    <main className="min-h-screen w-full bg-transparent pt-[90px] pb-12 px-4 md:px-12 text-slate-200 font-sans overflow-x-hidden">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="flex flex-col gap-3 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-2xl p-4 md:flex-row md:items-center md:justify-between">
          ...
          {activeDateFilter && (
            <div className="rounded-sm border border-white/10 bg-white/10 px-3 py-2 text-sm text-white/80">Filtering from {activeDateFilter.label}</div>
          )}
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              ...
              disabled={selectedIds.length === 0 || isMutatingRecords}
              variant="outlineFire"
              size="sm"
              className="gap-2"
            >
        ...
        <div className="space-y-3 md:hidden">
          ...
          {isRecordsLoading && (
            <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-2xl p-10 text-center text-slate-500">Loading records...</div>
          )}
          {recordsError && !isRecordsLoading && (
            ...
            {filterMode === 'month' ? (
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input type="date" value={rangeEnd} onChange={event => setRangeEnd(event.target.value)} className="bg-white/10 border-white/10 text-white" />
                </div>
              </div>
            )}
        ...
        <DialogContent className="bg-white/10 rounded-sm bg-clip-padding backdrop-filter backdrop-blur-md bg-opacity-30 border border-gray-100 text-white">
          ...
          <div className="space-y-4">
            ...
            <div className="max-h-[50vh] space-y-2 overflow-y-auto rounded-xl border border-white/10 bg-white/5 p-2">
              ...
              {fields.length === 0 && (
                <div className="rounded-lg border border-white/10 bg-white/5 p-4 text-center text-sm text-slate-500">No columns found.</div>
              )}
            </div>
          </div>
        </DialogContent>
