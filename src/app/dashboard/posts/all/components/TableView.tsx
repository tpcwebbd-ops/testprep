...
const ViewTableNextComponents: React.FC = () => {
    ...
    const renderActions = (item: IPosts) => (
        <div className="flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={() => { setSelectedPost(item); toggleViewModal(true); }}>
                <EyeIcon className="w-4 h-4" />
            </Button>
            <Button variant="outline" size="sm" onClick={() => { setSelectedPost(item); toggleEditModal(true); }}>
                <PencilIcon className="w-4 h-4" />
            </Button>
            <Button variant="destructive" size="sm" onClick={() => { setSelectedPost(item); toggleDeleteModal(true); }}>
    ...
    const renderTableRows = () =>
        sortedData.map((item: IPosts) => (
            <TableRow key={item._id}>
                ...
                <TableCell>{(item as any)['title']}</TableCell>
                <TableCell>{(item as any)['books']}</TableCell>
                    ...
                    <TableHeader className="bg-slate-100">
                        <TableRow>
                            ...
                            {tableHeaders.map(({ key, label }) => (
                                <TableHead key={key} className="cursor-pointer" onClick={() => handleSort(key as keyof IPosts)}>
                                    {label}{' '}
