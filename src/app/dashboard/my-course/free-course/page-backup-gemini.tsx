...
// import { useGetCoursesQuery } from '@/redux/features/course/courseSlice'; // Keeping your import
...
// |-----------------------------------------
// */
// // I'm using your provided JSON structure logic within the component
// // to ensure it works even without the actual API running for this view.
...
// type TaskType = 'VIDEO' | 'QUIZ' | 'UNKNOWN';
...
// // Helper to normalize content data (fixing the messy JSON structure)
// const parseContentItem = (item: CourseContentItem): { type: TaskType; payload: any } => {
...
//     // Handle specific JSON quirk where questions might be in item.data.questions
//     // OR inside nested numerical keys like item.data["0"]
//     let questions: Question[] = [];

//     if (Array.isArray(item.data.questions)) {
//       questions = item.data.questions;
//     } else {
//       // Search for nested objects containing questions
//       Object.values(item.data).forEach((val: any) => {
...
//   // Auto-complete after 5 seconds for demo purposes, or when video ends
//   useEffect(() => {
//     let timer: NodeJS.Timeout;
//     if (isPlaying) {
//       timer = setTimeout(() => {
//         // In a real app, you might wait for the full video
//         // onComplete();
//       }, 5000);
//     }
...
//       {/* Overlay controls for non-youtube links or just general UI */}
//       {!isPlaying && !youtubeId && (
//         <div
...
//       {/* Manual Complete Button (in case tracking fails) */}
//       <motion.button
...
//     if (isCorrect !== null) return; // Prevent changing after selection
...
//     } else {
//       setShowResult(true);
//       // Mark complete regardless of score for this demo, or enforce a pass mark
...
//                       ? 'bg-emerald-50 border-emerald-500 text-emerald-700' // Show correct answer if wrong selected
...
//   const { type, payload } = parseContentItem(item);
...
//           {type === 'VIDEO' && <VideoPlayer url={payload.url} onComplete={onComplete} />}

//           {type === 'QUIZ' && <QuizPlayer questions={payload.questions} onComplete={onComplete} />}
...
//         className={`absolute top-4 md:top-6 ${isLeft ? 'right-[calc(100%+1rem)] md:right-[calc(52%+4rem)] text-right' : 'left-[calc(100%+1rem)] md:left-[calc(52%+4rem)] text-left'} min-w-[140px] z-0 hidden sm:block`}
//       >
...
//   // Calculate local progress for this modal
...
//       setActiveTask(null); // Close the active task view
//     }
//   };
...
// const Page = () => {
//   // --- Data Fetching (Mocked for Demo Logic) ---
...
//   // Initialize completed content with some data (or empty)
//   // In a real app, this would come from a userProgress API
//   const [completedContentIds, setCompletedContentIds] = useState<string[]>([
//     // Pre-filling some IDs from your mock data so Day 1 looks partially done
//     'video-video-uid-1-1766560822726',
//   ]);

//   // --- Logic to Process Levels & Status ---
...
//     // 3. Determine Status (Locked/Current/Completed)
//     // The rule: A level is unlocked if the PREVIOUS level is FULLY COMPLETED.
//     // The first level is always unlocked.

//     let isPrevCompleted = true; // Start true for the first level
...
//       if (isComplete) {
//         status = 'completed';
//         isPrevCompleted = true; // Pass forward
//       } else if (isPrevCompleted) {
//         status = 'current';
//         isPrevCompleted = false; // Stop chain
//       } else {
//         status = 'locked';
...
//       // Optional: Check if this completion finished the whole course, trigger confetti?
//     }
//   };
...
//       {/* Modal handles the content and internal state */}
//       <ContentModal
