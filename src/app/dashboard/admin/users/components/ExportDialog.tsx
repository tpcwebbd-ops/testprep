/*
|-----------------------------------------
| setting up ExportDialog for the App
...
const downloadFile = (data: IUsers[], fileName: string) => {
...
const ExportDialog: React.FC<ExportDialogProps> = ({ isOpen, onOpenChange, headers, data, fileName }) => {
  ...
  const handleExport = () => {
    const selectedKeys = Object.keys(selectedColumns).filter(key => selectedColumns[key]) as (keyof IUsers)[];

    const processedData = data.map(row => {
      const newRow = {} as Partial<IUsers>;
      selectedKeys.forEach(key => {
        newRow[key] = row[key] as undefined;
      });
      return newRow as IUsers;
    });
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px] rounded-xl border border-white/20 bg-white/10 text-white backdrop-blur-2xl shadow-xl transition-all">
        ...
        <DialogFooter className="gap-2">
          <Button onClick={() => onOpenChange(false)} variant="outlineGlassy" size="sm">
            Cancel
          </Button>
          <Button onClick={handleExport} variant="outlineGlassy" size="sm">
            Export Data
          </Button>
        </DialogFooter>
      </DialogContent>
