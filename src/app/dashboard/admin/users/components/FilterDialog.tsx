/*
|-----------------------------------------
| setting up FilterDialog for the App
...
const FilterDialog: React.FC<FilterDialogProps> = ({ isOpen, onOpenChange, onApplyFilter, onClearFilter }) => {
      ...
      <Dialog open={isOpen} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-[425px] rounded-xl border border-white/20 bg-white/10 backdrop-blur-2xl shadow-xl transition-all text-white">
          <DialogHeader className="pb-3">
            <DialogTitle className="text-white bg-clip-text bg-linear-to-r from-white to-blue-200">Filter Users</DialogTitle>
            <DialogDescription className="text-white/70">Select a filter option to apply to the table data.</DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="mt-2 gap-2">
            <Button variant="outlineGlassy" onClick={handleClear}>
              Clear Filter
            </Button>
            <Button variant="outlineGlassy" disabled={isApplyDisabled} onClick={handleApply}>
              Apply Filter
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={isCalendarOpen} onOpenChange={handleCalendarClose}>
        <DialogContent className="p-2 rounded-xl border border-white/20 bg-white/10 backdrop-blur-xl shadow-lg md:p-4 max-w-[608px] min-w-[608px] w-[608px] text-white">
          ...
          <DialogFooter className="mt-4 flex justify-end gap-2">
            <Button variant="outlineGlassy" size="sm" onClick={handleCalendarClose}>
              Close
            </Button>
            <Button variant="outlineGlassy" size="sm" disabled={isCalendarUpdateDisabled} onClick={handleCalendarUpdate}>
              Update
            </Button>
          </DialogFooter>
        </DialogContent>
