...
export async function createCourse(req: Request): Promise<IResponse> {
  return withDB(async () => {
    try {
      ...
      const newCourse = await IELTScourse.create({
        ...courseData,
      });
...
export async function getCourseById(req: Request) {
  return withDB(async () => {
    ...
    const course = await IELTScourse.findById(id);
...
export async function getCourses(req: Request) {
  ...
  if (getValue) {
    ...
    return withDB(async () => {
      ...
      const courses = await IELTScourse.find(searchFilter).sort({ updatedAt: -1, createdAt: -1 }).skip(skip).limit(limit);

      const totalCourses = await IELTScourse.countDocuments(searchFilter);
...
export async function updateCourse(req: Request) {
  return withDB(async () => {
    try {
      ...
      const updatedCourse = await IELTScourse.findByIdAndUpdate(id, updateData, { new: true, runValidators: true });
  ...
  return withDB(async () => {
    ...
    const results = await Promise.allSettled(
      updates.map(({ id, updateData }: { id: string; updateData: Record<string, unknown> }) =>
        IELTScourse.findByIdAndUpdate(id, updateData, {
          new: true,
          runValidators: true,
        }),
      ),
    ...
    const successfulUpdates = results.filter(r => r.status === 'fulfilled' && r.value).map(r => (r as PromiseFulfilledResult<typeof IELTScourse>).value);
...
export async function deleteCourse(req: Request) {
  return withDB(async () => {
    ...
    const deletedCourse = await IELTScourse.findByIdAndDelete(id);
