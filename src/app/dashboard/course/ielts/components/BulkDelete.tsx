...
import { useBulkDeleteIELTSLecturesMutation } from '../redux/rtk-Api';
...
const BulkDeleteNextComponents: React.FC = () => {
  ...
  const [bulkDeleteCourses, { isLoading }] = useBulkDeleteIELTSLecturesMutation();
    ...
    <Dialog open={isBulkDeleteModalOpen} onOpenChange={toggleBulkDeleteModal}>
      <DialogContent>
        ...
        {bulkData?.length > 0 && (
          <div className="pt-4">
            <p>
              You are about to delete <span className="font-semibold">({bulkData.length})</span> courses
            </p>
          </div>
        )}
