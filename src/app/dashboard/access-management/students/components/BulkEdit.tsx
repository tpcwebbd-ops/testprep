...
import { useBulkUpdateUsers_student_accessMutation } from '../redux/rtk-Api';

import { handleSuccess } from './utils';

const BulkEditNextComponents: React.FC = () => {
  const { isBulkEditModalOpen, toggleBulkEditModal, bulkData, setBulkData } = useUsersAccessStore();
  const [bulkUpdateUsersAccess, { isLoading }] = useBulkUpdateUsers_student_accessMutation();
  ...
  const handleRoleChange = (UsersAccessId: string, role: string) => {
    setBulkData(bulkData.map(UsersAccess => (UsersAccess._id === UsersAccessId ? { ...UsersAccess, role } : UsersAccess)) as IUsers_access[]);
  };
            ...
            {bulkData.map((UsersAccess, idx) => (
              <div key={(UsersAccess._id as string) || idx} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4 min-w-[180px]">
                  ...
                  <Select onValueChange={role => handleRoleChange(UsersAccess._id as string, role)} defaultValue={(UsersAccess.role as string) || ''}>
