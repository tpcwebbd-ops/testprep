...
import { rolesSelectorArr } from '../store/store-constant';
import { useBulkUpdateRolesMutation } from '@/redux/features/roles/rolesSlice';
import { handleSuccess, handleError } from './utils';

const BulkUpdateNextComponents: React.FC = () => {
  const { toggleBulkUpdateModal, isBulkUpdateModalOpen, bulkData, setBulkData } = useRolesStore();
        ...
        {bulkData.length > 0 && (
          <div>
            ...
            <div className="w-full flex items-center justify-between pt-2">
              ...
              <Select onValueChange={value => handleFieldChangeForAll(value)} defaultValue={(rolesSelectorArr[0] as string) || ''}>
                <SelectTrigger className="bg-slate-50 w-[180px]">
                  <SelectValue placeholder="Select a value" />
                </SelectTrigger>
                <SelectContent className="bg-slate-50">
                  {rolesSelectorArr?.map((option, index) => (
          ...
          <div className="flex flex-col gap-2">
            {bulkData.map((role, idx) => (
              <div key={(role._id as string) || idx} className="flex items-center justify-between">
                ...
                <span className="text-blue-500">{role['name'] as string}</span>
              </div>
            ))}
          </div>
        </ScrollArea>
