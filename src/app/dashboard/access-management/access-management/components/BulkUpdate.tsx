...
const BulkUpdateNextComponents: React.FC = () => {
  const { toggleBulkUpdateModal, isBulkUpdateModalOpen, bulkData, setBulkData } = useAccessManagementsStore();
  ...
  const handleFieldChangeForAll = (value: string) => {
    setBulkData(
      bulkData.map(accessmanagement => ({
        ...accessmanagement,
        ['role']: value,
      })) as IAccessManagements[],
    );
  };
        ...
        {bulkData.length > 0 && (
          <div>
            ...
            <div className="w-full flex items-center justify-between pt-2">
              ...
              <Select onValueChange={value => handleFieldChangeForAll(value)} defaultValue={(accessManagementsSelectorArr[0] as string) || ''}>
                <SelectTrigger className="bg-slate-50 w-[180px]">
                  <SelectValue placeholder="Select a value" />
                </SelectTrigger>
                <SelectContent className="bg-slate-50">
                  {accessManagementsSelectorArr?.map((option, index) => (
          ...
          <div className="flex flex-col gap-2">
            {bulkData.map((accessmanagement, idx) => (
              <div key={(accessmanagement._id as string) || idx} className="flex items-center justify-between">
                ...
                <span className="text-blue-500">{accessmanagement['user_name'] as string}</span>
              </div>
            ))}
          </div>
        </ScrollArea>
