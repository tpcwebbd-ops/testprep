...
export default function StudentDashboard() {
  ...
  const students = useMemo(() => {
    // ⚠️ Correctly accessing the nested arrays from your API response structure
    ...
    // Define Base Dates (April 16, 2026 context)
    ...
    // 2. Map their course progress & calculate categories
    myCourses.forEach((mc: any) => {
      ...
      if (student) {
        ...
        // Attendance logic for Absent targeting exactly yesterday
          ...
          mc.attenDance.forEach((att: any) => {
            att.data?.forEach((d: any) => {
              if (d.status === 'complete' && d.completeDate) {
                ...
                if (dDate.getFullYear() === YESTERDAY.getFullYear() && dDate.getMonth() === YESTERDAY.getMonth() && dDate.getDate() === YESTERDAY.getDate()) {
    ...
    return Array.from(studentMap.values()).map(student => {
      ...
      const attendedYesterdayOverall = student.courses.some((c: any) => c.attendedYesterday);

      if (hasFailed) {
        overallCategory = 'failed'; // Failed has priority
      } else if (allComplete || isEnrollmentComplete) {
        overallCategory = 'complete';
      } else if (isRunning) {
        overallCategory = attendedYesterdayOverall ? 'running' : 'absent';
      } else {
    ...
    <main className="min-h-screen bg-slate-950 p-4 font-sans text-slate-100 md:p-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <SummaryCard title="Absent (Yesterday)" value={summary.absent} icon={<AlertTriangle />} color="bg-amber-500/10 text-amber-400 border-amber-500/30" />
          <SummaryCard
...
Now your task is update the absend status. If a student is not complete any class in last 50 hour then it is concidare as absend. if attend in last 50 hour then he will conciedare as running. 