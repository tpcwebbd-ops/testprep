...
import { X, UploadCloud, Loader2, Ghost, Search, CheckCircle2, Zap, FileEdit, Files, ChevronLeft, ChevronRight, Plus, FileDown } from 'lucide-react';
...
interface InternalVaultProps {
  ...
  selectedDocxs: string[];
}

const InternalDocxVault = ({ onDocxToggle, selectedDocxs }: InternalVaultProps) => {
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              placeholder="SCAN DOCX REPOSITORY..."
          ...
          <div className="hidden">
            <DialogTitle>Docx Library Vault</DialogTitle>
            <DialogDescription>Multi-node document management</DialogDescription>
          </div>
        </div>
      </DialogHeader>
      ...
      <ScrollArea className="flex-1 p-8">
        {isFetching ? (
          <div className="flex flex-col items-center justify-center py-40 gap-6">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-white/20 animate-pulse">Syncing Library...</span>
          </div>
        ) : availableDocs.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
            <AnimatePresence mode="popLayout">
              {availableDocs.map((item, idx) => {
                const isSelected = selectedDocxs.includes(item.url);
                return (
                  <motion.div
                    key={item._id}
                    layout
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 p-6 border-t border-white/5 bg-white/5">
        ...
        <div className="flex items-center gap-4 w-full md:w-auto">
          ...
          <Button
            ...
            className="w-full md:w-auto h-12 px-8 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-black uppercase tracking-[0.2em] text-[10px] transition-all flex items-center justify-center gap-3 border-none"
          >
            {isUploadingLocal || isAdding ? <Loader2 className="w-4 h-4 animate-spin" /> : <UploadCloud className="w-4 h-4" />}
            {isUploadingLocal || isAdding ? 'ARCHIVING...' : 'IMPORT WORD DATA'}
          </Button>
        </div>
      </div>
    </div>
...
export default function DocxUploadManager({
  value,
  onChange,
  label = 'WORD DOCUMENT COLLECTION',
}: {
  value: string[];
    ...
    <div className="space-y-6 w-full">
      <div className="flex items-center justify-between px-2">
        <div className="space-y-1">
          ...
          <p className="text-[8px] font-bold uppercase tracking-widest text-blue-400">{value.length} Assets Selected</p>
        </div>
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          <DialogTrigger asChild>
            <Button
              variant="outlineGlassy"
              size="sm"
              className="rounded-2xl border-white/10 bg-white/5 hover:bg-blue-500/10 hover:border-blue-500/50 transition-all gap-2 group h-9 px-5 text-[9px] font-black uppercase tracking-widest"
            >
        ...
        <ScrollArea className="w-full">
          {value.length > 0 ? (
              ...
              <AnimatePresence mode="popLayout">
                {value.map((url, index) => (
                  <motion.div
                    ...
                    className="group relative aspect-[3/2] rounded-3xl overflow-hidden bg-black/40 border border-white/10 hover:border-blue-500/30 transition-all duration-500 flex flex-col items-center justify-center"
                  >
                    <FileEdit className="w-8 h-8 text-blue-400/40 group-hover:text-blue-400 group-hover:scale-110 transition-all" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col justify-end p-5">
                      <div className="flex items-center justify-between">
                        <a
                          ...
                          className="p-2.5 bg-blue-500/20 backdrop-blur-md rounded-xl border border-blue-500/30 hover:bg-blue-500 transition-colors"
                        >
            ...
            <div className="flex flex-col items-center justify-center py-12 opacity-20 gap-6">
              ...
              <p className="font-black uppercase tracking-[0.5em] text-[11px] text-center">Word Processor Standby</p>
            </div>
          )}
        </ScrollArea>
      </div>
