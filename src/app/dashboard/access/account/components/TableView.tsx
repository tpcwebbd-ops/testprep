...
import { useGetAccountsQuery } from '@/redux/features/accounts/accountsSlice';
...
type DisplayableAccountsKeys = 'accountId' | 'providerId' | 'userId' | 'accessToken' | 'idToken' | 'accessTokenExpiresAt' | 'scope' | 'createdAt';
type ColumnVisibilityState = Record<DisplayableAccountsKeys, boolean>;

const ViewTableNextComponents: React.FC = () => {
  const [open, setOpen] = useState(false);
  const [sortConfig, setSortConfig] = useState<{
    key: DisplayableAccountsKeys;
    direction: 'asc' | 'desc';
  } | null>(null);
  ...
  const allData = useMemo(() => getResponseData?.data?.accounts || [], [getResponseData]);

  const tableHeaders: { key: DisplayableAccountsKeys; label: string }[] = useMemo(
    () => [
      ...
      { key: 'accessTokenExpiresAt', label: 'Access Token Expires At' },
  ...
  const handleSort = (key: DisplayableAccountsKeys) => {
  ...
  const handleSelectRow = (isChecked: boolean, item: IAccounts) => setBulkData(isChecked ? [...bulkData, item] : bulkData.filter(i => i._id !== item._id));

  const renderActions = (item: IAccounts) => {
    return (
  ...
  return (
    <div className="w-full flex flex-col">
      ...
      <ExportDialog
        ...
        data={bulkData}
        fileName={`Exported_Accounts_${new Date().toISOString()}.xlsx`}
      />
    </div>
  );
};
