...
import { useBulkDeleteAccountsMutation } from '@/redux/features/accounts/accountsSlice';
...
const BulkDeleteNextComponents: React.FC = () => {
  const { isBulkDeleteModalOpen, toggleBulkDeleteModal, bulkData, setBulkData } = useAccountsStore();

  const [bulkDeleteAccounts, { isLoading }] = useBulkDeleteAccountsMutation();
      ...
      <DialogContent className="sm:max-w-md rounded-xl border border-white/20 bg-white/10 backdrop-blur-2xl shadow-xl text-white">
        ...
        {bulkData?.length > 0 && (
          <p className="text-white/80 mt-2">
            You are deleting&nbsp;
            <strong>({bulkData.length})</strong> accounts.
          </p>
        )}
