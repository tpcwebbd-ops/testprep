look at the roles/TableView.tsx 
```
'use client';
...
and here is access/TableView.tsx 
```
'use client';
...
const ViewTableNextComponents: React.FC = () => {
  ...
  const renderTableRows = () =>
    sortedData.map((item: IAccessManagements, idx: number) => (
      <TableRow key={item._id || 0 + idx}>
        <TableCell>
          <Checkbox onCheckedChange={checked => handleSelectRow(!!checked, item)} checked={bulkData.some(i => i._id === item._id)} />
        </TableCell>
        {visibleHeaders.map((header, idx) => (
          <TableCell key={header.key + idx}>{header.key === 'createdAt' ? formatDate(item.createdAt) : String(item[header.key] ?? '')}</TableCell>
        ))}
        <TableCell className="text-right max-w-[10px]">{renderActions(item)}</TableCell>
      </TableRow>
    ));
  ...
  return (
    <div className="w-full flex flex-col">
      ...
      <div className="max-w-xs flex items-center self-center justify-between pl-2 gap-4 border rounded-sm w-full mx-auto mt-8">
        <Label htmlFor="set-limit" className="text-right text-slate-200 font-normal pl-3">
          Data per page
        </Label>
        <Select
          onValueChange={value => {
        ...
        >
          ...
          <SelectContent>
            {pageLimitArr.map((i, idx) => (
              <SelectItem key={i + idx} value={i.toString()} className="cursor-pointer">
                {i}
              </SelectItem>
            ))}
          </SelectContent>
...
Now copy responsive from roles/TableView.tsx and implement it in access/TableView.tsx 
and remember do not change color combination and others style. 