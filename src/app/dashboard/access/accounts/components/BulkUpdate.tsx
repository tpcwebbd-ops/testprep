...
const BulkUpdateAccounts: React.FC = () => {
  const { toggleBulkUpdateModal, isBulkUpdateModalOpen, bulkData, setBulkData } = useAccountsStore();
        ...
        {bulkData.length > 0 && (
          <div className="space-y-3">
            <p className="pt-2 text-white/80">
              You are about to update <span className="font-semibold text-white">({bulkData.length})</span> accounts.
            </p>
            ...
            <div className="flex items-center justify-between rounded-lg p-3 bg-white/5 border border-white/10 backdrop-blur-md">
              ...
              <Select onValueChange={value => handleFieldChangeForAll(value)} defaultValue={(accountsSelectorArr[0] as string) || ''}>
        ...
        <ScrollArea className="h-[300px] w-full rounded-xl border border-white/10 bg-white/5 backdrop-blur-md p-4 mt-3">
          <div className="flex flex-col gap-2">
            {bulkData.map((item, idx) => (
              ...
              >
                <span>
                  {idx + 1}. {(item['accountId'] as string) || ''}
                </span>
