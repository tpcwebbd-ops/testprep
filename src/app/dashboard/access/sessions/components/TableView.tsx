...
import { useGetSessionsQuery } from '@/redux/features/sessions/sessionsSlice';
...
type DisplayableSessionsKeys = 'expiresAt' | 'token' | 'createdAt' | 'updatedAt' | 'ipAddress' | 'userAgent' | 'userId';
type ColumnVisibilityState = Record<DisplayableSessionsKeys, boolean>;

const ViewTableNextComponents: React.FC = () => {
  const [open, setOpen] = useState(false);
  const [sortConfig, setSortConfig] = useState<{
    key: DisplayableSessionsKeys;
    direction: 'asc' | 'desc';
  } | null>(null);
  ...
  const allData = useMemo(() => getResponseData?.data?.sessions || [], [getResponseData]);

  const tableHeaders: { key: DisplayableSessionsKeys; label: string }[] = useMemo(
    () => [
  ...
  const handleSort = (key: DisplayableSessionsKeys) => {
  ...
  const handleSelectRow = (isChecked: boolean, item: ISessions) => setBulkData(isChecked ? [...bulkData, item] : bulkData.filter(i => i._id !== item._id));

  const renderActions = (item: ISessions) => {
    return (
  ...
  return (
    <div className="w-full flex flex-col">
      ...
      <ExportDialog
        ...
        data={bulkData}
        fileName={`Exported_Sessions_${new Date().toISOString()}.xlsx`}
      />
    </div>
  );
};
