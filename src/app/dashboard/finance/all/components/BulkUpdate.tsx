...
const BulkUpdateNextComponents: React.FC = () => {
  ...
  const handleFieldChangeForAll = (value: string) => {
    setBulkData(
      bulkData.map((finance: IFinances) => ({
        ...finance,
        ['role']: value,
      })) as IFinances[],
    );
        ...
        {bulkData.length > 0 && (
          <div>
            ...
            <div className="w-full flex items-center justify-between pt-2">
              ...
              <Select onValueChange={value => handleFieldChangeForAll(value)} defaultValue={(financesSelectorArr[0] as string) || ''}>
                <SelectTrigger className="bg-slate-50 w-[180px]">
                  <SelectValue placeholder="Select a value" />
                </SelectTrigger>
                <SelectContent className="bg-slate-50">
                  {financesSelectorArr?.map((option, index) => (
          ...
          <div className="flex flex-col gap-2">
            {bulkData.map((finance: IFinances, idx: number) => (
              <div key={(finance?._id as string) || idx} className="flex items-center justify-between">
                ...
                <span className="text-blue-500">{finance['studentName'] as string}</span>
              </div>
            ))}
          </div>
        </ScrollArea>
