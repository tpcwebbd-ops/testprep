  ...
  const generateFormFieldJsx = (key: string, type: string): string => {
    ...
    switch (typeName.toUpperCase()) {
      ...
      case 'DYNAMICSELECT':
        ...
        break;
      case 'IMAGE':
        componentJsx = `<ImageUploadManagerSingle value={new${singularPascalCase}['${key}']} onChange={(url) => handleFieldChange('${key}', url)} />`;
        break;
      case 'IMAGES':
...
This generate-add.ts  is responsible for generating Add.tsx File.
Here is some information that you have to remember. 
1. those are all Values = 'STRING, EMAIL, PASSWORD, PASSCODE, SELECT, DYNAMICSELECT, IMAGES, IMAGE, DESCRIPTION, INTNUMBER, FLOATNUMBER, BOOLEAN, DATE, TIME, DATERANGE, TIMERANGE, COLORPICKER, PHONE, URL, RICHTEXT, AUTOCOMPLETE, RADIOBUTTON, CHECKBOX, MULTICHECKBOX, MULTIOPTIONS, STRINGARRAY'

2. this is Value and Make input field is readonley if found "#readonly'
3. if found "SELECT#Bangladesh, India, Pakistan, Canada" here SELECT is value and "Bangladesh, India, Pakistan, Canada" are options. same as "RADIOBUTTON#OP 1, OP 2, OP 3, OP 4" and "MULTIOPTIONS#O 1, O 2, O 3, O 4"

Now Your task is implement those features in generate-add.ts 
1. some time inputJsonFile looks like 
  ```
  {
    "uid": "0b5323b0-f9cf-4fe0-9d9c-f7259435d610",
    "templateName": "Batches",
    "schema": {
      "name": "STRING"
    },
    "namingConvention": {
      "Users_1_000___": "Batches",
      "users_2_000___": "batches",
      "User_3_000___": "Batch",
      "user_4_000___": "batch",
      "use_generate_folder": false,
      "bulk_action": []
  ...
  2. look at the authImport. it check Email#readonly. if found then render import if not found then render nothing. now implement all checking for all Values. 
