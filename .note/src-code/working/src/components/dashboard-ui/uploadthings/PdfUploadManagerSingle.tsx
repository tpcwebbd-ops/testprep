...
import { X, Loader2, FileText, Ghost, RefreshCcw, Search, CheckCircle2, ShieldCheck, Layers } from 'lucide-react';
...
interface InternalPdfVaultProps {
  onPdfSelect: (newPdf: string) => void;
  selectedPdf: string;
}

const InternalPdfVault = ({ onPdfSelect, selectedPdf }: InternalPdfVaultProps) => {
      ...
      <ScrollArea className="flex-1 p-8">
        {isFetching ? (
          <div className="flex flex-col items-center justify-center py-32 gap-6">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-white/20 animate-pulse">Scanning File Systems...</span>
          </div>
                ...
                availablePdfs.map((item: any, idx: number) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`relative aspect-[3/4] rounded-2xl overflow-hidden border-2 cursor-pointer transition-all duration-500 group flex flex-col items-center justify-center gap-4 p-4
                      ${isSelected ? 'border-indigo-500 bg-indigo-500/10 scale-95 shadow-[0_0_40px_rgba(99,102,241,0.2)]' : 'border-white/5 bg-white/[0.02] hover:border-white/20 hover:scale-105'}
                    `}
                    >
                      <FileText
                        className={`w-12 h-12 transition-colors duration-500 ${isSelected ? 'text-indigo-400' : 'text-white/20 group-hover:text-white/40'}`}
                      />
                      <span className="text-[8px] font-black uppercase tracking-widest text-white/60 text-center line-clamp-2 px-2">
                        {item.name || 'DOCUMENT_UNNAMED'}
                      </span>
                      {isSelected && (
          ...
          <UploadButton
            ...
            onClientUploadComplete={async res => {
              if (res && res[0]) {
                ...
                onPdfSelect(res[0].url);
                toast.success('Document uplink synchronized');
              }
            }}
            onUploadError={err => {
              setIsUploadingLocal(false);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between px-1">
        ...
        {value && (
          ...
          >
            <X className="w-3 h-3 mr-1" /> Remove Source
          </Button>
        )}
      </div>
            ...
            {value ? (
              <div className="relative w-full h-full flex items-center justify-center">
                <div className="flex flex-col items-center gap-4">
                  ...
                  <div className="px-6 py-2 rounded-2xl bg-white/10 border border-white/20 text-[10px] font-black uppercase tracking-[0.2em] text-white">
                    Asset Secured
                  </div>
                </div>
                ...
                <div className="absolute bottom-6 left-6 right-6 flex items-center justify-between pointer-events-none">
                  <div className="px-4 py-2 bg-indigo-500/20 backdrop-blur-md border border-indigo-500/30 rounded-full flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-black text-indigo-300 italic tracking-tighter">ENCRYPTED_UPLINK</span>
                  </div>
                </div>
              </div>
            ) : (
              ...
              >
                <div className="w-16 h-16 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center group-hover:scale-110 group-hover:bg-indigo-500/10 group-hover:border-indigo-500/30 transition-all duration-500">
                  <FileText className="w-8 h-8 text-white/20 group-hover:text-indigo-400" />
                </div>
