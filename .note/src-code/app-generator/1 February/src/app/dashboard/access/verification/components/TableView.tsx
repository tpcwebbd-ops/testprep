...
const ViewTableNextComponents: React.FC = () => {
  ...
  const [sortConfig, setSortConfig] = useState<{
    key: DisplayableVerificationsKeys;
    direction: 'asc' | 'desc';
  } | null>(null);
  ...
  const allData = useMemo(() => getResponseData?.data?.verifications || [], [getResponseData]);

  const tableHeaders: { key: DisplayableVerificationsKeys; label: string }[] = useMemo(
    () => [
  ...
  const handleSort = (key: DisplayableVerificationsKeys) => {
  ...
  const handleSelectRow = (isChecked: boolean, item: IVerifications) => setBulkData(isChecked ? [...bulkData, item] : bulkData.filter(i => i._id !== item._id));

  const renderActions = (item: IVerifications) => {
    return (
  ...
  const renderTableRows = () =>
    sortedData.map((item: IVerifications) => (
  ...
  return (
    <div className="w-full flex flex-col">
      ...
      <ExportDialog
        ...
        data={bulkData}
        fileName={`Exported_Verifications_${new Date().toISOString()}.xlsx`}
      />
    </div>
  );
};
