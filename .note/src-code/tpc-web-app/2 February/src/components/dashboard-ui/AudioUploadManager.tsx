...
import { Plus, X, Loader2, Music, Ghost, Search, CheckCircle2, Layers, Zap, Headphones, Play, Volume2 } from 'lucide-react';
            ...
            <div className="flex items-center gap-3">
              {(isUploadingLocal || isAdding) && (
                <div className="flex items-center gap-2 px-4 py-2 bg-indigo-500/10 rounded-full border border-indigo-500/20">
                  ...
                  <span className="text-[9px] font-black uppercase tracking-widest text-indigo-400">Syncing Frequencies</span>
                </div>
              )}
              <UploadButton
                ...
                onClientUploadComplete={async res => {
                  if (res && res[0]) {
                    ...
                    onAudioToggle(res[0].url);
                    toast.success('Frequency node active');
                  }
                }}
                onUploadError={err => {
                  setIsUploadingLocal(false);
      ...
      <ScrollArea className="flex-1 p-8">
        {isFetching ? (
          <div className="flex flex-col items-center justify-center py-32 gap-6">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-white/20 animate-pulse">Scanning Waves...</span>
          </div>
                ...
                availableAudios.map((item: any, idx: number) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`relative aspect-square rounded-2xl overflow-hidden border-2 cursor-pointer transition-all duration-500 group flex flex-col items-center justify-center p-4
                    ...
                    >
                      ...
                      />
                      <span className="text-[8px] font-black uppercase tracking-widest text-white/70 text-center line-clamp-2 px-2">
                        {item.name || 'AUDIO_STREAM'}
                      </span>
                      {isSelected && (
          ...
          <div className="flex flex-col items-center justify-center py-32 opacity-20 space-y-6">
            ...
            <div className="text-center">
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest mt-2">No detectable audio signatures</p>
            </div>
          </div>
        )}
      </ScrollArea>
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between px-1">
        <div className="space-y-1">
          ...
          <p className="text-[8px] font-bold uppercase tracking-widest text-white/10">{value.length} Waves Locked</p>
        </div>
              ...
              >
                <div className="relative aspect-video bg-white/5 border border-white/10 flex flex-col items-center justify-center backdrop-blur-xl shadow-2xl rounded-xl group-hover:border-indigo-500/30 transition-all duration-700 p-6">
                  <div className="relative flex items-center justify-center mb-2">
                    ...
                    >
                      {[1, 2, 3].map(i => (
                        <div key={i} className="w-0.5 bg-indigo-400/40 rounded-full" />
                      ))}
                    </motion.div>
                  </div>
                  <span className="text-[9px] font-black text-white/40 uppercase tracking-widest text-center truncate w-full px-4">ENCRYPTED_SIGNAL</span>

                  <div className="absolute inset-0 bg-slate-950/60 opacity-0 group-hover:opacity-100 transition-all duration-500 flex items-center justify-center">
          ...
          {[1, 2].map(i => (
            ...
            >
              ...
              >
                ...
                <span className="text-[10px] font-black text-slate-50/80">Empty Band</span>
              </motion.div>
