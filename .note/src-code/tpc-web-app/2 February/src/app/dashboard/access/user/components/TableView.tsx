...
type DisplayableUsersKeys = 'name' | 'email' | 'emailVerified' | 'createdAt';
...
const ViewTableNextComponents: React.FC = () => {
  ...
  const allData = useMemo(() => getResponseData?.data?.users || [], [getResponseData]);

  const tableHeaders: { key: DisplayableUsersKeys; label: string }[] = useMemo(
    () => [
      { key: 'name', label: 'Name' },
  ...
  const handleSelectRow = (isChecked: boolean, item: IUsers) => setBulkData(isChecked ? [...bulkData, item] : bulkData.filter(i => i._id !== item._id));
