...
/**
 * A component that displays a summary of accessManagements data in a dialog.
 * It features a trigger button, data fetching with RTK Query,
 * and a paginated table view of monthly statistics.
 */
const AccessManagementsSummary = () => {
  const [page] = useState(1);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const limit = 10;

  const { data, isLoading, isError } = useGetAccessManagementsSummaryQuery(
    { page, limit },
  ...
  return (
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      ...
      <DialogContent className="sm:max-w-4xl">
        <DialogHeader>
          <DialogTitle>AccessManagements Summary</DialogTitle>
          <DialogDescription>An overview of all accessManagements data, aggregated by month.</DialogDescription>
        </DialogHeader>
...
export default AccessManagementsSummary;
