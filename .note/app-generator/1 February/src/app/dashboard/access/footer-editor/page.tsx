...
const SkeletonCard = () => (
  <div className="bg-[#0f0f0f]/60 backdrop-blur-md border border-white/5 rounded-2xl overflow-hidden shadow-xl p-6 flex flex-col md:flex-row items-center justify-between gap-6 animate-pulse">
    ...
    <div className="flex gap-3">
      <div className="h-9 w-24 bg-neutral-700 rounded-xl" />
      <div className="h-9 w-24 bg-neutral-700 rounded-xl" />
    </div>
  </div>
);
...
const Modal = ({
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 top-[10px] md:top-0">
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
...
const FooterManager = () => {
  ...
  const handleSelectTemplate = async (key: string) => {
    ...
    toast.promise(updateFooter(updatedFooter).unwrap(), {
      ...
      error: {
        render({ data }) {
          return `Import failed: ${getErrorMessage(data)}`;
        },
      },
    });
  };
  ...
  const handleSubmitSettings = async () => {
    if (!formData?.name?.trim()) {
      toast.error('Validation Error: Friendly Name is required.');
      return;
    }
    ...
    toast.promise(mutationPromise, {
      pending: isEdit ? 'Updating settings...' : 'Creating new footer...',
      success: {
        render() {
          setIsFormOpen(false);
      ...
      error: {
        render({ data }) {
          return `Failed to save: ${getErrorMessage(data)}`;
        },
      },
    });
  };
  ...
  const handleListToggle = async (footer: FooterItem) => {
    ...
    toast.promise(updateFooter({ ...footer, isEnabled: newState }).unwrap(), {
      ...
      error: {
        render({ data }) {
          return `Failed: ${getErrorMessage(data)}`;
        },
      },
    });
  };
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        >
          ...
          <div className="flex gap-3 w-full md:w-auto">
            <Button onClick={() => refetch()} variant="secondary" className="px-3" disabled={isLoading}>
        ...
        {/* Content Area - Handling Loading/Error/Data */}
        <div className="grid grid-cols-1 gap-6">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 border border-dashed border-red-500/20 rounded-2xl bg-red-500/5">
              <AlertCircle size={48} className="text-red-500 mb-4" />
              <h3 className="text-lg font-medium text-white">Failed to load footers</h3>
              <p className="text-neutral-500 mb-4">The server connection might be unstable.</p>
